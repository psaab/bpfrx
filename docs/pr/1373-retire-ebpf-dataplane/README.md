...
| `pkg/daemon/dataplane_facade.go` | #5275 the revocable handle every EXTERNAL consumer (gRPC/REST/CLI) holds instead of a raw backend alias, so dropping the dataplane actually reaches them. It restates the union of the `runtime_probes.go` mirrors, so it names the same root `pkg/dataplane` types for the same reason and moves when they do. |
| `pkg/daemon/dataplane_facade_wiring.go` | #5275 `setDataplane` keeps `d.dp` and the external consumers' facade in lockstep (and revokes the outgoing facade); names `dataplane.RuntimeDataPlane` only as the parameter type. |
| `pkg/daemon/dataplane_facade_probes.go` | #5275 daemon-side mirrors of the RUNTIME capability probes the external consumers type-assert against the facade — cursor session iteration, applied-NAT view (#5794), policy-scheduler state (#3062), userspace status/control. Unlike the `runtime_probes.go` mirrors these have no assignment site, so the compile-time assertions here are the only build-time proof the facade satisfies them. The cursor mirror names root `pkg/dataplane` session types for the same reason `runtime_probes.go` does and moves when they do. |
