...
| `pkg/cli/top_talkers_bound_8597.go` | #8597 K05 — `show security flow session sort-by bytes|packets` reads `dataplane.SessionKey`/`SessionValue` (and the v6 twins) to build a bounded top-K heap. The console runs IN `xpfd` with no recover and `MaxSessions` is dynamic and large (#5323), so the previous per-session materialisation let an unprivileged show command OOM the control plane on the box being diagnosed. Bounding the COLLECTION rather than the print is what requires the session types here. Render-only, no legacy enforcement path. |
