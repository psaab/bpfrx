...
| Uptime | `Uptime:` | Primary: `/proc/self/stat` starttime converted via `/proc/stat` btime — same anchor the CPU row uses. Fallback (if /proc reads fail): in-memory process start time passed as the `startTime` arg to `fwdstatus.Build` (gRPC handler passes `s.startTime`; local TTY handler passes the existing CLI `c.startTime`, which is CLI-construction time — close enough for fallback display since it's only hit when State = Unknown anyway). → `time.Since` formatted "N days, N hours, N minutes, N seconds" | Sharing the PID-start anchor with the CPU row keeps the two numbers consistent when /proc is readable. The implementation did not need to add a new `daemonStartTime` field on the CLI struct. |
