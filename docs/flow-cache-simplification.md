...
every neighbor refresh and collapse the fast-path hit rate). All three
...
accumulating on the single global epoch, and the three resolver cases
(first-insert no-bump, change bumps — fail-on-revert for the resolver —,
epoch-reject no-bump). `cached_descriptor_evicted_only_on_neighbor_mac_change`
