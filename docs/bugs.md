...
- **Fix (pkg/config/compiler_security_addressbook.go):** Mirror the `address` merge-by-name — fetch-or-create the `AddressSet` by name, then UNION members via `appendUniqueString` (first-seen order, dedup) matching Junos union-by-name semantics. Nested `address-set` references took the same overwrite path and are fixed too. A single-stanza set compiles identically (no regression).
