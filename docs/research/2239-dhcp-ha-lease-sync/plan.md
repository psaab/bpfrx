...
> **IMPLEMENTED** (PATH C). Shipped on `feat/2239-dhcp-ha-lease-sync`.
> Open-question resolutions chosen (per the SMR + plan recommendations):
> Q1 = incremental on-grant push (a 2s change-detect alongside the 30s
> full-set heartbeat); Q2 = socket-first read with memfile fallback;
> Q3 = pre-seed the Kea memfile BEFORE start AND `lease{4,6}-add` after
> (fully closes the duplicate-allocation window per SMR NIT-1); Q4 = explicit
> `set chassis cluster dhcp-lease-synchronization` knob; Q5 = full v6
> DUID/IAID/`type`/`prefix-len` (IA_PD synced); Q6 = mutual NTP left as
> documented optional hardening (remaining-lifetime re-anchor removes the hard
> dependency); Q7 = full lease re-push on sync (re)connect via the
> `OnPeerConnected` nudge. The standby's Kea stays STOPPED
> (`clearRethServicesForRG` unchanged) — VRRP/RG is the sole who-serves
> arbiter, exactly the IPsec-SA-sync model.
