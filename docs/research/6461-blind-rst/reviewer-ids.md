...
| r100 | PLAN NO (4B/1H/1M) — suppression/clean-miss stragglers; OverdueSkipped field definition + poller carriage (install_failed hoist at :509, reduction at :883); invalidation lifecycle (immediate current-binding + sibling fan-out at every site-2c transition incl. initial construction and non-probation-predecessor replacement); parity phrasing contradictions; inertness over-scoped (job task-ms97csej-11i9cu) | SOUND at r99 (v10.16.0, no new findings) | PLAN YES (r100 fold verification → r101 doc) | v10.17.0 (field definition + carriage + composition + teardown guards; invalidation lifecycle; text reconciliation) |
| r101 | PLAN NO (5B/1H/1M) — OverdueSkipped producer/transport contract still prose; composition/teardown guards not in the normative list; displaced-identity SET missing (simultaneous placeholder-shadow + predecessor-replace discards both); invalidation timing not mechanically realizable as written; cold-seed timeout artifact; engagement phrasings (job task-ms9835jf-wg0xep) | SOUND at r100 (v10.17.0) | PLAN YES (r101 fold verification → r102 doc) | v10.18.0 (full producer/transport/consumer contracts; displaced-identity set; realizable timing; artifact removals) |
| r102 | PLAN NO (5B/2H/2M) — normative contract absent (v10.17/18 edits landed in summary paragraphs; §5.8 outcome list never gained the field/enum/composition; constructor sites + pre-promotion ordering unspecified); EMPTY-set unsound in both refusal meanings; invalidation timing unrealizable as written; zero-alloc capacity missing; chain qualifications + unscoped claims (job task-ms98oy28-q47ajr) | SOUND at r101 (v10.18.0) | PLAN YES (r102 fold verification → r103 doc; notes the silent-replace process defect, fixed with assertion-checked folds) | v10.19.0 (the single normative §5.8 contract block; ValidatorRefused/UpsertRefused; WorkerScratch accumulator + poll_binding fan-out; all text scopes) |
| r103 | PLAN NO (4B/2H/1M) — set carrier not realizable (inline-vs-accumulator split needed); refusal outcomes undefined + capacity misclassification (synced upsert refuses only non-peer predecessors, install.rs:310-315); OverdueSkipped non-empty when a placeholder was shadowed; §9 SSOT contradiction + missing refusal tests; last overclaims (job task-ms99jr2y-s66884) | SOUND at r102 (v10.19.0, every named site verified) | PLAN YES (r103 fold verification → r104 doc) | v10.20.0 (six-variant outcome incl. ValidatorRefused/UpsertRefused with UpsertRefused gating the same authority consumers; inline DisplacedSet + WorkerScratch batch accumulator split; placeholder identity rides the set; all stragglers scoped) |
| r103 (AGY) | — | UNSOUND (2 findings: §5.6 EMPTY-set straggler; §5.8 bullet-(i) omitted UpsertRefused from the teardown enumeration — everything else verified sound) | — | v10.20.1 (both folded) |
