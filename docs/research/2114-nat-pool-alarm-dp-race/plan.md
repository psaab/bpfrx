...
- **Prior**: DRAFT v90 — r88 folds: AGY b1's missing §9 test
  ...
  hook after the entry Store(false), a concurrent IsLoaded /
  REST `health.go:107` / gRPC `server_show_status.go:22` read
  observes `DataplaneLoaded==false` DURING the window where
  master would still report true); AGY b2's arithmetic +
  terminology correction (v89's summary said "13 + 3 = 16"
  while its own body enumerated FOURTEEN if-ok sites — the
  mixed subset is 14 if-ok + 3 nil-guard = 17 sites, and it is
  now explicitly scoped as the MIXED / MULTI-ACCESS subset of
  the ~41 total neutral-outcome accesses; the SINGLE-MAP
  NEUTRAL SET is named — the twelve stale cleanups at
  `maps_stale.go:18/:41/:65/:93/:117/:148/:178/:201/:262/:309/
  :348/:371`, `maps_counters.go:181`, `maps_stats.go:72` — plus
  the mixed-method neutral-returns at `maps_nat.go:400/:435`,
  `maps_policy.go:253`, `maps_session.go:612`, `loader.go:910/
  :928`, `maps_counters.go:233`); AGY's oracle verification
  (ClearNAT64Configs' chain VERIFIED DISCRIMINATING) and
  per-access gating audit (no residual method-gated phrasing)
  are fold-free; r88 verdicts: Codex PENDING (task-msbc0207-
  pjmnbd), AGY PLAN-NEEDS-MINOR (2 blocking minors), Claude SMR
  PENDING; pending Codex r88 + convergence review r89.
- **Prior**: DRAFT v89 — r87 folds: Codex M1's inventory
  ...
  iface_zone_map nil-guard at `loader.go:700`; v89 called the
  count "13 + 3 = 16" — itself miscounted, corrected at v90 per
  r88 AGY b2 to 14 + 3 = 17 mixed sites) and the
...
- **Revision history**: v1 @ `1d62be758` (initial). v2 @ `61568128f` (r1:
  ...
  correction to its own follow-up). v80 (r78: the class-text
  consistency rewrite (one-state sentences deleted); the
  ...
  verified a misread, not folded). v82 (r80: the class-2
  predicate corrected to the neutral set; the carve-out's
...
- `pkg/dataplane/loader.go`: `loaded bool` → `loaded atomic.Bool`
  (:36 declaration; Store(true) as the FINAL step INSIDE the
  whole-batch `m.mu` hold, after all population — batch+flag
  publish atomically; `:458`, `:490`, `:1082` become Load;
  Store(false) at `Close()`'s ENTRY (:1206), before the
  link-handle closes).
- **THE CONTRACT — a TOTAL, EXCLUSIVE partition of the exported
  ...
  :38`); methods touching only `m.mu`-protected Go state are
  category G; lifecycle and facade methods are categories L/F. The §9 matrix test assigns every
  ...
  - **Class 1 — fallible map-required methods, no required pre-gate
    ...
    precedence on an unarmed manager). Mechanically (the uniform
    form, r78 Codex M1's consistency fix — the v70-era one-state
    sentence is deleted): the method enters the registry helper;
    under `m.mu` it classifies (fresh / retained / armed) and
    selects the handle ATOMICALLY — during the whole-batch hold it
    BLOCKS (it does not observe false and skip); after release it
    observes the armed state (Store(true) is the batch's final
    in-hold step). The happens-before edge: the batch's population
    sequenced-before the in-hold release-Store of `loaded=true`
    ...
    **THE GATE PREDICATE IS TWO-STATE (r75 Codex M1 — the fresh-vs-
    ...
    class proceeds EXACTLY as master — with the PRE-EXISTING
    LOADED-CHECK SET carved out (r77 Codex M3 + r78 Codex M3):
    `AttachXDP`/`AttachTC` carry master's own pre-registry
    rejection (`loader.go:490`, `:1082` — "eBPF programs not
    loaded"), AND the `CompileConfig` path rejects
    `!dp.IsLoaded()` with "dataplane not loaded"
    (`compiler.go:182`) before registry access — reached by
    `Manager.Compile` (`compiler.go:316`), `ApplyConfig`
    (`apply.go:237`), and `CompileUserspaceShim` via the embedded
    Manager (`loader.go:173`); `Compile` touches `m.maps` directly
    at `compiler.go:353`, so it is inside the classified surface
    and its master rejection is preserved. These fire on BOTH
    states and are preserved as-is; "proceeds exactly as master"
    applies only to methods WITHOUT a pre-existing loaded check,
    and for this set master's rejection IS the behavior (the
    class-1 typed error never fires for them): retained reads report the
  ...
  - **Class 2 — neutral-outcome methods, ANY signature, WITH the
    ...
    contradicted the under-lock classification and would again
    suppress retained state); on FRESH return master's missing-map
    outcome
...
- The RACE-1/RACE-2 closure claims stay the two precise layers
  ...
  `loader.go:1221-1235`), the retained handles reference LIVE
  FD-BACKED OBJECTS OF AN OBSOLETE FORWARDING GENERATION (Teardown
  does not close the `m.maps` FDs, r80 Codex m1's precision — a
  mutation can SUCCEED yet miss the new generation), a re-`Start`
  creates FRESH maps
  ...
  EXACT recurrence class the follow-up's work item H terminates —
  with the generic lifecycle/generation redesign assigned to its OWN
  follow-up issue (r77 Codex m2, §10 — the standalone timeout path
  `daemon_apply_commit.go:645` also Teardown-retains and re-arms, and
  H is FirstCommit+cluster-scoped)
...
- The `m.mu` contract comment at `loader.go:49` (r71 Codex m2,
  aligned with §4 at v75 per r73 Codex m2, scoped to the uniform
...
12. **Armed-state admission (work item A3, r68 Codex M1; scoped r69
    ...
    registry rule — master's exact behavior (r79 Codex M3 +
    r80 Codex M2's precision: methods WITH a pre-existing loaded
    check — the attaches, the CompileConfig path — reject
    WHENEVER `loaded==false`, i.e. on BOTH unarmed states, and
...
4a. A3 armed-gate tests (`pkg/dataplane`, r68 Codex M1 / r69 Codex m1 /
   ...
   fresh-outcomes-during-hold oracle alive and is DELETED):
   (1) quiescent FRESH outcomes (every class, no overlap);
   (2) quiescent RETAINED outcomes (every class proceeds as
   master, seeded retained registry, no overlap); (3) the blocked
   FRESH-Start lock-ownership leg (readers block during the hold;
   observe armed after release); (4) the blocked RETAINED-re-Start
   lock-ownership leg (same, on the retained fixture) — both
   blocked legs observe ARMED after release because Store(true) is
   the batch's final in-hold step; (5) the ISLOADED-WINDOW leg
   (v90 addition per r88 AGY b1 — the v89 text claimed "the §9
   teardown legs assert this directly" but named NO such leg):
   `TestManager_ArmedGate_CloseWindowIsLoaded` — an armed
   Manager's `Close()` is held at a test hook placed immediately
   AFTER the entry `Store(false)` (`loader.go:1206`) and BEFORE
   the link-handle closes; a concurrent `Manager.IsLoaded()` read
   (and, through the userspace adapter
   `legacy_dataplane.go:86`, the REST `statusHandler`'s
   `DataplaneLoaded` at `health.go:107` and gRPC `GetStatus`'s at
   `server_show_status.go:22` — the direct-callable surface)
   ...
   made per-entry-point exact at v82 per r80 Codex M3): a direct
   `Manager.Compile`/`ApplyConfig` call rejects IMMEDIATELY even
   pre-Store (its first registry-adjacent step is `CompileConfig`'s
   loaded check); a direct `CompileUserspaceShim` runs the two
   legacy cleanups (:174/:177) and the constant selector write
   (:181 — citation corrected from :180, r80 Codex m2) BEFORE the
   ...
   blocked re-`Start` seam). TWO further guards (r80 Codex M4 —
   the class-3 hybrids already take `m.mu` for their side effects
   BEFORE their raw lookup — `ClearGlobalCounters` locks at
   `maps_counters.go:176` and looks up at :181 — so the
   writer-first held-batch test blocks class 3 at the PRE-EXISTING
   ...
   *ebpf.Program, present bool, st registryState)`, where
   ...
   (snat_rules), `:148` (snat_rules_v6), `:178` (dnat_table),
   `:201` (dnat_table_v6), `:262` (nptv6_rules), `:309`
   (screen_configs), `:348` (iface_filter_map), `:371`
   (filter_configs) — all `!ok → return` in void methods; plus
   `maps_counters.go:181` (`ClearGlobalCounters`' global_counters
...
- **Pre-existing retained-generation confusion on the teardown-
  ...
  the retained `m.maps`/`m.programs` handles reference live
  FD-backed objects of an obsolete forwarding generation (r80 Codex
  m1 — Teardown does not close the FDs) while re-`Start` creates
  fresh ones. A
...
7. **r68-r88 resolution (for the record)**: Codex r68 M1 (armed-state
   ...
   outcome matrix TOTAL, r86 made it PER-ACCESS, r87 corrected
