...
`IMPLEMENTED (#3607)` — Option B shipped on branch
`fix/3607-screen-tokenbucket` (base origin/master `dd271a118`). The converged
v6 design below was implemented as specified: a monotonic-ns fixed-point
`TokenBucket` (`userspace-dp/src/screen/rate.rs`, 16 B, `ONE = 1e9`,
`refill_q = elapsed_ns * threshold`, `saturating_sub` clock delta capped at 1 s,
`saturating_add` before the `.min(capacity)` clamp, cold-start FULL), migrating
the ICMP/UDP flood per-zone aggregates, the standby SYN-cookie ACK validation
budget, and the SYN-flood aggregate DROP path when `syn-cookie` is OFF (sole
drop authority, decoupled from the measured `over_attack`, alarm explicitly
gated on `!over_attack`). The count-all `RateCounter` aggregate (cookie
activation + alarm), the #3315 sketch, and the missing-profile warn dampener
are unchanged. `now_ns` is threaded from the batch-cached `loop_now_ns` through
`stage_screen_check` / the standby-ACK stage into the screen `_opts` variants
and `validate_syn_cookie_ack_on_session_miss`; the second-granularity
convenience wrappers derive `now_secs * 1e9`. Full `cargo test --release`
green (3673 passed / 0 failed); RED-on-revert verified (cookie-OFF
sustained-at-threshold admits 100/1500 on the count-all revert vs ≥99% with the
bucket). Docs updated (`docs/syn-cookie-flood-protection.md`, module docs).
The #3315 per-source/per-destination sketch #3607 over-throttle (§5b) remains a
tracked follow-up.

Original `/research` status below (retained for the design record).
