...
template). [SHIPPED]** Completes #2679 for consumer/SaaS DNS.
...
- **Done:** all four backends are siblings of the rfc2136 backend behind the
  SAME `DNSUpdater` interface (`pkg/ddns/backend_{dyndns2,cloudflare,route53,
  generic}.go` + the shared `backend_http.go`); the Route 53 SigV4 signer is
  minimal/self-contained (`sigv4.go`, no AWS SDK — we sign one API shape).
  `productionSurfaceABackend` (surface_a.go) is the single backend resolution
  point keyed on `DDNSProvider.Backend`; an HTTP backend with a missing
  credential degrades to the no-op (fail-open, the commit warning fired).
  Credentials are all `config.Secret`; HTTPS with cert+hostname verification,
  bounded timeout, capped body. checkip is opt-in per provider (`checkip-url` +
  `checkip-allowlist`) with the inadyn validity gate. Tests are mock-server
  (httptest) driven through the real backends — `dyndns2` good/nochg/abuse
  parse, Cloudflare create/update/delete + bad-token-auth, Route 53 UPSERT/
  DELETE change-batch + SigV4 known-vector, generic template render +
  success-regex, checkip validity gate + allowlist, engine-through-HTTP publish/
  skip/backoff, and a no-secret-in-logs/show assertion.
- **Deferred to the parent (lab gate):** a LIVE publish against a real provider
  (CI has no provider creds / outbound network). The HA gate is reused unchanged
  from P1b/P2, so P3 is not HA-specific; the parent may run `make test-failover`
  for no-regression but it is not P3-specific.
