...
The primary deliverable is out-of-process wire verification across live Incus endpoints (`trust-host`, `untrust-host`, `dmz-host`, `nat64-host`, `nptv6-host`). Every row requires positive execution evidence before checking invariants and defines an explicit observable failure:
...
| **NAT64 (RFC 6146)** | IPv6 probe targeting synthesized `64:ff9b::10.0.2.102` | Translated IPv4 datagram emerges on untrust; return mapped to IPv6 client | `FAIL: nat64_untranslated (synthesis drop)` | `wire_nat_nat64` |
| **NPTv6 (RFC 6296)** | IPv6 UDP probe with ULA source `fd01:203:405:bf02::102` | 1:1 prefix rewrite to GUA; 1s-complement checksum-neutral header transit | `FAIL: nptv6_checksum_corrupted (bad L4 checksum)` | `wire_nat_nptv6` |
...
- **Deterministic Provisioning:** `test/incus/setup.sh up` provisions Layer 2 bridges (`xpf-trust`, `xpf-untrust`, `xpf-dmz`) and enforces DUT isolation via `assert_sole_dataplane_owner()` (#1992/#1961).
