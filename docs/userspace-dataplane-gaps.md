...
| WireGuard inner-ingress (decapped plaintext) | **Partially closed by #5618.** `wg_control/policy_gate.rs` now applies the xpf FORWARD zone policy to decapped inner packets before the `wgN` TUN write: from-zone from the tunnel's LOGICAL interface (zone + routing instance, the native-GRE-decap model), to-zone from the xpf-FIB-resolved egress interface, `evaluate_policy` on the inner 5-tuple, non-permit → drop. Before #5618 the plaintext went straight to the kernel FIB, so a DENIED inter-zone flow transited — while the same policy WAS enforced on the transit-egress direction (LAN → `wgN`), making the bypass silently one-directional. **Residual (tracked on #5618, still open):** inner ingress creates no xpf session, runs no NAT/PBR/filter/screen, emits no RT_FLOW policy-deny, takes no host-inbound admission, and is not HA-RG gated — those need the bounded control-thread→worker logical-ingress handoff. Where xpf cannot compute a zone pair (unzoned tunnel or egress, host-bound inner destination, unroutable inner, unparseable inner) the kernel delegation is preserved and COUNTED (`inner_policy_unadjudicated` / `xpf_userspace_wg_inner_zone_policy_total{verdict="unadjudicated"}`) rather than silent. See `docs/wireguard-interop.md` "Inner-ingress zone-policy authority". |
