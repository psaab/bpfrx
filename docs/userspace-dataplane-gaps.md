...
| IPsec / XFRM handling | Userspace detects and punts to kernel/slow-path as needed. **#5619 (open):** the kernel-decapsulated `xfrmi`/`if_id` PLAINTEXT then traverses Linux routing with no xpf zone-policy consumer, so inter-zone authority for IPsec-passthrough inner traffic rests with the kernel. Not covered by the #5618 WireGuard gate below — there the plaintext exists in a userspace-dp buffer that xpf can decline to write, while here `xfrm_input` materializes it inside the kernel and hands it to the routing stack without ever entering userspace-dp. |
| WireGuard inner-ingress (decapped plaintext) | **Partially closed by #5618.** `wg_control/policy_gate.rs` now applies the xpf FORWARD zone policy to decapped inner packets before the `wgN` TUN write: from-zone from the tunnel's LOGICAL interface (zone + routing instance, the native-GRE-decap model), to-zone from the xpf-FIB-resolved egress interface, `evaluate_policy` on the inner 5-tuple, non-permit → drop. Before #5618 the plaintext went straight to the kernel FIB, so a DENIED inter-zone flow transited — while the same policy WAS enforced on the transit-egress direction (LAN → `wgN`), making the bypass silently one-directional. A packet with no parseable 5-tuple is adjudicated on the L3 identity it DOES carry (src, dst, protocol) via the shared `evaluate_policy_result_l3_aware(.., l4_present = false)` path the transit direction already uses (#3291/#4569) — NOT delegated: `parse_flow_ports` yields no 5-tuple for every IP protocol except TCP/UDP/identifier-bearing-ICMP, for every ICMP error and ND/MLD type, and for every non-first fragment, so delegating that class would have left the deny bypassable by an authenticated peer setting one byte (AllowedIPs constrains the inner SOURCE, nothing constrains the inner PROTOCOL NUMBER). A discard route, an IPv6 extension chain that is OVER-LIMIT (the #4743 IDS-evasion shape) or TRUNCATED (an extension header declaring a length past the end of the packet), a packet DECLARING TCP or UDP that carries no readable L4 header (mainline's shim resolves TCP/UDP eagerly in `parse_l4` and XDP_DROPs on failure; a NON-FIRST FRAGMENT is exempt, since it legitimately has no L4 header and is adjudicated on L3), and a non-IP / sub-L3-length payload DROP and count as `inner_forward_drops` — deliberately narrow: an IPv6 `NoNextHeader` terminal (59) is a legal chain terminal with no L4, so it is adjudicated flowlessly on its L3 identity, not dropped, and is stamped with protocol 59 rather than the base header's Next Header byte (which is an extension-header number when the 59 sits behind one). The TRUNCATED case is a drop because the only protocol byte still available IS an extension-header number, so no protocol- or port-keyed rule can match it — one bad length byte would otherwise walk a packet through a DENY. That is mainline PARITY, not over-reach: on the transit path the XDP shim drops the truncated shape at INGRESS (`parse_ipv6` revalidates the declared length, returns `None`, `drop_degraded_transit(.., PARSE_FAIL)` → `XDP_DROP`), so it never reaches the userspace stage whose #4743 comment says a truncated packet "keeps its existing flowless handling" — that sentence is about a stage this shape does not reach. A port-bearing PERMIT matches the first fragment but not the non-first fragments, which fall to the default policy — the documented #3291 fail-closed limitation inherited from the mainline transit path, whose deferred fragment-association cache closes it in both directions at once (#4569 covers only skipped port-bearing DENIES, never PERMITS). One sub-case is NOT parity and should not be described as such: for a flow xpf same-family NATs, mainline's non-first fragments INHERIT the first fragment's permitted verdict through the #5689 association (`nat_consult_forward_fragment_assoc`) and so do not fall to the default policy. That mechanism is structurally unreachable here — `nat_install_forward_fragment_assoc` self-gates on the committed decision carrying a same-family rewrite and runs only at the AF_XDP cold-path forward commit, a path inner plaintext never takes (it goes to the TUN and the kernel routes it), so xpf commits no NAT decision and installs no association, and a consult added to this gate would miss unconditionally. Closing it needs the same control-thread→worker handoff as the rest of the residual, not a lookup this gate can make on its own. **Residual (tracked on #5618, still open):** inner ingress creates no xpf session, runs no NAT/PBR/filter/screen, emits no RT_FLOW policy-deny, takes no host-inbound admission, and is not HA-RG gated — those need the bounded control-thread→worker logical-ingress handoff. Kernel delegation is preserved and COUNTED (`inner_policy_unadjudicated` / `xpf_userspace_wg_inner_zone_policy_total{verdict="unadjudicated"}`). NOT peer-steerable (fixed by the tunnel row's `logical_ifindex`): unzoned tunnel interface, endpoint absent during teardown. OPERATOR-GATED and family-selected: the tunnel's unit carries an input filter with a `then routing-instance` (filter-based-forwarding) term, which overrides the table the egress lookup should use. That override is real for this traffic — `pkg/routing/rules.go` installs the kernel ip rule in the 31000-31999 band ahead of the main table so "the kernel also honors PBR for XDP_PASS'd packets", and #5117 scopes each rule to the ingress interface (`FRA_IIFNAME`), which for decapped plaintext is `wgN` — so the gate's base-table to-zone is not where the packet actually leaves, in either direction (a denied base-table zone would blackhole traffic the override VRF permits; a permitted one would wave through traffic the override VRF's zone pair denies). The gate evaluates no filters, so it delegates rather than adjudicate on the wrong zone pair. A peer cannot CREATE this branch (it needs the operator's filter), but the precheck is per-family and the peer picks the inner version nibble, so on a tunnel with FBF configured for one family it can select that family. PEER-STEERABLE via the inner destination but delegated for MAINLINE PARITY — `NoRoute` and `NextTableUnsupported` are both slow-path ELIGIBLE (`ForwardingDisposition::is_slow_path_eligible`) and the mainline transit policy gate runs only for `ForwardCandidate`, so dropping them in the WireGuard direction alone would diverge from that contract and re-create the asymmetry this gate removes. PEER-STEERABLE and genuinely open: a host-bound inner destination (`LocalDelivery`) belongs to the host-inbound plane (`host-inbound-traffic` admission + `to-zone junos-host`), which this control-thread gate does not implement — AllowedIPs constrains the inner source only, so a peer may freely address any firewall-local IP (unchanged from master; closing it is full-fix scope). An unzoned EGRESS interface is NOT delegated: the peer selects it, so it is adjudicated with `to_zone = 0` and resolved by #3110's default-action fall-through, exactly as mainline does. See `docs/wireguard-interop.md` "Inner-ingress zone-policy authority". |
