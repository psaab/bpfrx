...
| **Policy Rematch** | `security policies policy-rematch` | Re-evaluate existing sessions when policy changes | Medium | **Mostly enforced (#4234 / #4342 / #4343).** All halves ship in `pkg/daemon/daemon_policy_invalidate.go` (keyed on the session `policy_id` #3056, propagated to the HA peer via the #2468 delete-sync, run under `d.applySem`). **Ordering (#6948):** the candidate sessions are READ in one pass immediately BEFORE the dataplane publishes the new policy snapshot (`capturePolicyInvalidationLocked`, called at the last statement before `rt.ApplyConfig` in `applyDataplaneAndHACore`), and the deletes are issued from that capture after the apply. Runtime policy ids are positional, so the new snapshot renumbers them and a survivor can inherit a deleted policy's id; the live row's `policy_id` then stops meaning what the diff meant, both because new sessions are admitted under the new ids and because the helper's #3395 live-row refresh (`refresh_bpf_conntrack_last_seen`, a 2048-slot slice every 100ms with a 10s full-table target) re-resolves established rows to the new numbering — a deleted policy's rows to `DEFAULT_POLICY_SENTINEL_ID`, a survivor's to its new id. Reading the table after the apply therefore swept the sessions of whichever policy inherited a deleted id and missed the deleted policy's own. A path that reaches the invalidation without an armed plan (a boot apply) falls back to the legacy post-apply scan plus the `policyActivationSecs` creation-time guard, which is a strict subset of the capture's protection, never a disable. (1) The Junos-DEFAULT **deletion-clear** is always on: a session admitted by a policy that a commit DELETES is invalidated immediately at commit, independent of the knob. (2) The **modified-policy re-evaluation** runs when `policy-rematch` is set: a surviving policy (same zones+name) whose MATCH or ACTION changed has its live sessions cleared at commit so the tightened policy re-evaluates traffic (`changedPolicyRuntimeIDs` / `clearSessionsForModifiedPolicies`; identity via `PolicyIDsByStableKey`, match/action diff via `PoliciesByStableKey`; overloaded `policy_id 0` excluded). (3) **Default-policy change (#4342):** a change to the IMPLICIT default-policy re-evaluates the live default-PERMIT sessions, which carry `DefaultPolicySentinelID` (0xFFFFFFFF) rather than any configured id (`defaultPolicyChanged` / `clearSessionsForDefaultPolicyChange`). A verdict change (permit↔deny/reject) clears them UNCONDITIONALLY like the deletion-clear; a session-log flip (`default-policy-log session-init/session-close`) clears them only under `policy-rematch`. The 0xFFFFFFFF sentinel can never alias `policy_id 0`, so the id-0 host-inbound/fabric/synced fail-safe does not apply here. (4) **Scheduler-binding change (#4343):** because `policyRuleInactive` is FAIL-CLOSED (an inactive-scheduled policy is skipped in the rule chain), a surviving policy whose scheduler-name binding or scheduler active-WINDOW flips it active→inactive is treated as a verdict change and its sessions are cleared under `policy-rematch` (`policySchedulerBecameInactive`, driven by the per-scheduler active-state evaluated via `policySchedulerActiveStateForApplyLocked`). (5) **`policy-rematch extensive` (#8993):** Junos re-evaluates even sessions of an UNCHANGED policy when a referenced address-book / application object changes. `changedPolicyRuntimeIDs` gained a third arm, `policyReferencedObjectChanged`, which compares the RESOLVED form of each policy — `dpuserspace.PolicyResolvedFingerprints`, a hash of the very `PolicyRuleSnapshot` the dataplane is sent — rather than its match/action text. Tightening an address-set, redefining an address, or changing an application definition therefore re-evaluates the sessions of every policy that references it, with no policy text changed. It is gated on the sub-mode: a plain `policy-rematch` does not build the fingerprints and behaves exactly as before. The snapshot is sufficient on its own because its address lists are RESOLVED PREFIXES and its book IDs are CONTENT hashes (`addressBookContentHash64`), so no separate address-book comparison is needed — an earlier draft that added one was measured inert and removed. The advisory that used to accompany the knob is GONE, the same way the bare knob's went when its core shipped. Residual: a dynamic-address FEED refresh is not a commit, so it does not reach this path at all. **Error contract (#5578):** the shared core `clearSessionsForPolicyIDs` and its three wrappers RETURN an aggregated `error` (both families always attempted, errors joined). A PARTIAL invalidation — a failed `ForEachV4/V6` enumerate leaves unvisited sessions in the live table, a failed `DeleteBatchKnownV4/V6` leaves matched sessions installed — is a stale-authorization gap (traffic the new policy should now DENY keeps forwarding on old session state), so the returning callers join it into the result: the commit path (`applyAndSyncCommitted`) surfaces it alongside the non-fatal `applyErr` (mark-and-continue — config stays committed + still syncs to the peer, operator sees the failure and can re-commit), the peer sync-recv path (`syncAndApply`) returns it to `handleConfigSync`, and the void confirm-timeout rollback logs it loudly. The `slog.Error/Warn` diagnostics are kept but the failure is no longer swallowed by a log line alone. |
...
| Feature | Junos Config Path | Description | Priority | Status |
|---------|-------------------|-------------|----------|--------|
| **Proxy ARP for NAT** | `security nat proxy-arp interface ... address ...` | Auto-reply ARP for NAT pool addresses on same subnet as ingress interface. Required when SNAT pool, DNAT, or static-NAT external addresses are on same L2 segment. | High | **Done** -- Proxy ARP neighbor entries (NTF_PROXY) for NAT addresses with GARP on addition, AND the per-interface `net.ipv4.conf.<if>.proxy_arp` sysctl is enabled so the kernel actually answers the ARP (#2160). The kernel has two ARP-proxy paths: the pneigh (NTF_PROXY) reply branch, which answers only when the target routes out a *different* interface and does not consult the sysctl; and the `arp_fwd_proxy` path, gated by the per-interface `proxy_arp` sysctl. **#8621 CORRECTION, measured: enabling the sysctl does NOT guarantee a reply, and for the same-L2-subnet case it changes nothing.** `arp_fwd_proxy` opens with `if (rt->dst.dev == dev) return 0;` -- it bails BEFORE it reads `IN_DEV_PROXY_ARP`. So for a target inside the connected subnet of the interface the request arrives on, all three arms of `arp_process`'s proxy branch return 0: `arp_fwd_proxy` on that first line, the pneigh arm on its own `rt->dst.dev != dev` guard, and `arp_fwd_pvlan` on a `proxy_arp_pvlan` sysctl nothing sets. Measured on the loss userspace cluster: a probe for a pool address left the requester's neighbour entry INCOMPLETE while the interface's own address answered 2/2 REACHABLE from the same host in the same second; setting only `proxy_arp_pvlan=1` made it answer and reverting stopped it. The sysctl remains correct and load-bearing for a target routed out a DIFFERENT interface, which is what `arp_fwd_proxy` exists for -- the error was the word "guarantees". **The same-L2 case is now answered by a userspace responder** (`pkg/cluster/arp_responder_8621.go` + `pkg/daemon/daemon_proxyarp_responder_8621.go`): the daemon opens AF_PACKET/ETH_P_ARP per configured interface and replies for exactly the configured addresses, gated per REQUEST on redundancy-group ownership so a standby never answers with its own per-node RETH virtual MAC. **#8640: that gate reads the CLUSTER's per-RG election state** (`cluster.GroupState(rgID).State == StatePrimary`, the same predicate `shouldOwnDirectVIPs` uses to decide whether this node holds the RG's VIPs at all), NOT the VRRP-event-derived `rgStateMachine.Ownership()`. The latter answers `Unknown` as a steady state on a real cluster — `vrrpInstances` is fed only by VRRP state-change events and nothing seeds it from current state — and measured on the loss userspace cluster it reported `vrrp-master=none` for every redundancy group while the same output said `node0 primary / node1 secondary`, so ten fresh ARP probes for a pool address were answered by BOTH nodes. Reading the election state makes one invariant hold: a node answers proxy-ARP for an address on an interface exactly when it holds that interface's VIPs, because both questions read the same predicate. The VRRP-derived path remains the fallback where the cluster manager cannot answer, so a standalone node is unaffected. Note: with the default `medium_id=0`, `proxy_arp=1` makes the kernel answer ARP on that interface for ANY target routed out a different interface -- broader than Junos `proxy-arp` (which proxies only listed addresses); per-address (v4-only) narrowing is **deferred** in #2197 item 3 (PLAN-DEFER, lab characterization pending -- dropping the sysctl would re-break the same-L2 #2160 case). IPv6 addresses get a real proxy-NDP pneigh install (#2197 item 1, see Proxy NDP row). The reconcile is also re-asserted by an always-on 30s ticker so a non-commit kernel link cycle (HA RETH flap, `programRethMAC`) self-heals without an operator re-commit (#2197 item 2). Removing proxy-arp from an interface now also DISABLES the sysctl: the daemon remembers the last-enabled `(iface -> families)` set and writes `proxy_arp`/`proxy_ndp` back to `0` for any pair that drops out on a later commit, so the over-broad responder no longer leaks on across the config removal until reboot (#2475). A proxy-arp entry on a VLAN sub-interface (`reth0.50`, `ge-0/0/0.100`) resolves to that sub-interface's OWN VLAN netdev ifindex (`cfg.ResolveKernelIfName`, which maps the unit to its 802.1Q VLAN ID and collapses unit 0 onto the bare parent), not the parent -- `proxy_arp`/`proxy_ndp` are per-netdev, so the old parent-ifindex resolution wrote the sysctl on the parent and left the VLAN sub-interface silent (#3010). Config: `set security nat proxy-arp interface <iface> address <addr>` with range support. A multi-host PREFIX (`address 198.51.100.0/29`) is expanded to its ARP-addressable hosts at compile time (#6559), excluding the network and broadcast addresses; a /31 keeps both per RFC 3021. Before #6559 the prefix length was silently dropped and the whole block installed ONE entry for the address as authored -- for a canonical prefix that is the network address, so the block answered nothing while the `proxy_arp` sysctl still read as enabled. The expansion is capped at 256 hosts, the same cap `address <low> to <high>` already carried; a larger block is rejected at commit (warned on the tolerant load path, #1960) rather than silently truncated. |
...
| **Session Limiting (dest-ip)** | `security screen ids-option ... limit-session destination-ip-based N` | Limit max concurrent sessions to single destination IP | High | **Done** (#2134) -- same per-IP `SessionTable` mechanism as source-ip limiting, keyed on the destination IP (`session-limit-dst`). Same per-worker multiplier applies (#2186): effective cap ≈ `N × configured`. **The same multiplier applies to every screen RATE threshold** — syn-flood attack/alarm/source/destination, icmp/udp flood, port-scan, ip-sweep and the cookie-OFF bucket all live in the per-worker `ScreenState` (#6568 member 3, documented in `docs/syn-cookie-flood-protection.md` “Limitations”). |
...
| **Alarm Without Drop (audit mode)** | `security screen ids-option <name> alarm-without-drop` | Profile-wide audit/log-only mode: a tripped check still ALARMS (logs the attack, counts it) but does NOT drop the packet — the standard vSRX step for tuning screen thresholds against live traffic. | Medium | **Done** (#4170) -- profile-wide flag on `ScreenProfile.AlarmWithoutDrop` (Go + Rust wire). The dataplane verdict consumers (`stage_screen_check` in `poll_stages.rs` for the flowless L3 + flow-path screens, and the scan/sweep + session-limit site in `poll_descriptor`) convert a `ScreenVerdict::Drop` (and the flow-path `SynCookieChallenge`) into a log-only PERMIT alarm carrying the tripped reason (via `emit_screen_alarm_event`, mirroring the #3315 `syn-flood-alarm` path) and FORWARD the packet. The check still runs, counts its sketch/tracker state, and logs; only the drop is suppressed. Applies to every check in the profile including the rate-based flood / SYN-cookie paths. For the SYN-cookie path this required gating the cookie-active marking (`syn_cookie_active_until_secs`) on `!alarm_without_drop` in `check_packet`: audit mode never mints cookies on the wire, so marking the zone cookie-active would arm `validate_syn_cookie_ack_on_session_miss` to DROP every returning session-miss ACK as `Invalid` -- escaping the audit contract. Leaving the zone inactive makes that path return `NotApplicable` so the ACK forwards; the non-audit SYN-cookie flow is unchanged. A profile with ONLY alarm-without-drop and no enabled check publishes no snapshot; **#9425** made the modifier survive that. Two paths lost it. (1) **INERT zone:** such a zone has no `ScreenState::zones` entry, so `alarm_without_drop(zone)` returned false — and since #7888 the dataplane substitutes `conservative_default()` and HARD-DROPS the malformed-packet set for it. `set security screen ids-option p alarm-without-drop` and nothing else is both the canonical route into the inert state and an explicit request for audit mode, so the operator received the exact inverse of what they configured, on a commit that reported success with zero warnings. The flag now rides on the inert wire reference (`ScreenInertProfileRefs.AlarmWithoutDrop`, `alarm_without_drop` in `screen_inert_profile_zones`), and `ScreenState::alarm_without_drop` falls back to it when the resolved lookup misses — so the substituted checks still RUN, still count `substituted_default_drops`, still WARN, and now ALARM instead of dropping. #7888's decision is preserved: it chose drop over PASS, and this is drop vs ALARM. An UNDEFINED reference is deliberately not covered — there is no profile that could have declared the modifier. The protocol version moved 10 -> 11 for it, because an old helper that ignores the field keeps hard-dropping and hard-dropping IS the defect. (2) **ENFORCE -> AUDIT transition:** `syn_cookie_active_until_secs` was inherited across `update_profiles` (#4969 preserves per-zone state; #2446 invalidates only the cookie GENERATION), so a zone latched cookie-active under a live flood kept `locally_active` true after the operator committed `alarm-without-drop` TO STOP THE DROPS, and `validate_syn_cookie_ack_on_session_miss` — which never consulted `alarm_without_drop` — kept dropping parse-valid session-miss ACKs as `Invalid` for up to `EPOCH_SECS` (64 s) after that commit. `reconcile_substate` now clears the latch when the new profile is in audit mode; that is idempotent with the freshly-configured-audit guard above, since audit mode never sets the latch. A non-audit profile edit still inherits it. |
...
| **TCP Closing Timeout** | `security flow tcp-session closing-timeout N` | Idle timeout for a TCP session that has begun closing | Medium | **DONE (#7342)** — carried on the wire (`tcp_closing_timeout`) into `SessionTimeouts.tcp_closing_ns`, and it now governs a state distinct from TIME_WAIT: a FIN observed in ONE direction. Unset keeps the 30s `TCP_CLOSING_TIMEOUT_NS` every post-FIN close used before the split. A RST abort is unchanged on the 2s `TCP_RST_TIMEOUT_NS` (#3046), which is an xpf control rather than a Junos leaf. |
| **TCP Time-Wait Timeout** | `security flow tcp-session time-wait-timeout N` | Idle timeout for a session in TIME_WAIT | Low | **DONE (#7342)** — the leaf needed a STATE before it could need a carrier: `session_timeout_ns` split a close only into RST and not-RST, so there was no TIME_WAIT window distinct from `closing-timeout`'s. `SessionEntry` now tracks a FIN per DIRECTION (mirrored between the forward and reverse halves by `propagate_tcp_state_to_companion`), so a close with a FIN in both is TIME_WAIT and one with a FIN in one is CLOSING. Unset keeps the same window `closing-timeout` reports, so an operator who sets neither leaf reaps exactly as before the split. |
...
| Feature | Junos Config Path | Description | Priority | Status |
|---------|-------------------|-------------|----------|--------|
| **Filter change does not revoke established sessions** | `interfaces <if> unit <u> family inet filter input <name>` | Junos firewall filters are stateless and evaluated on EVERY packet, so a newly attached or tightened input filter applies to established flows immediately | Medium | **CLOSED for a purely static filter (#7212)**: an established session is revalidated against a changed STATIC address/protocol/port input filter on its next packet and revoked if the filter now denies it. Each session entry carries the config generation its input-filter verdict was computed under; on a hit whose stamp predates the live snapshot the static verdict is re-derived ONCE, side-effect free (`NonRoutingCountPolicy::Never` — no `then count`, no `then log`), and the session is revoked only on DENY, so a still-PERMITTED flow — including its SNAT translated port — is never touched. That is the difference from the interface-keyed clear #5858 rejected. A filter whose verdict varies per packet (DSCP #1430 / per-packet L4 #2362) keeps its existing per-packet re-evaluation. Revocation tears down forward + reverse (#5622), releases the NAT reservation once, emits the forward close delta, and evicts both directions' flow-cache slots on every binding of the worker in the same tick; sibling workers evict on their next tick via the existing `DeleteSynced` path (#6457). A peer-SYNCED session imports with stamp `0`, which is never a live generation, so a promoted session revalidates against the local filter state before it forwards. The #5858 commit-time advisory was deleted with the fix. NOT closed for: a filter containing any `routing-instance` term (the non-routing walk defers on a matched PBR term and cannot tell the deferral from an Accept, so #7212 declines rather than preserve a session #4392 would drop); a resolved decision with no local session entry (the #2120 transient synced hit, and a reverse-NAT repair refused at `max_sessions`); and the HA resync half of #7212's own list — a `SessionSync.BulkSync` fallback for a denied set that overflows the per-worker Close ring. Tracked as #8114 and #8108. |
...
| **Schedulers** | `class-of-service schedulers ...` | Define per-queue scheduling parameters (transmit rate, priority, drop profile) | Low | Partial (userspace supports transmit-rate, `transmit-rate exact`, priority, buffer-size, `surplus-sharing` (#915 — non-Junos extension that opts an `exact` queue into surplus-phase participation while keeping its per-queue rate as a guarantee floor), and `equal-flow-enforcement` (#1304 — non-Junos opt-in, mutually exclusive with `surplus-sharing`, that lets shared exact v8 leases suppress faster workers toward the slowest sampled per-active-SFQ-bucket grant rate). `transmit-rate percent <n>` (#4228 Gap 2) and `transmit-rate remainder` (#6846) both RESOLVE to an absolute byte/sec rate per bound interface — percent against the interface's shaping-rate, remainder against what resolved siblings leave — each retaining a narrowed commit advisory for the cases that still cannot resolve (no shaping base; a leftover that floors to zero). The fuller Junos scheduler/drop-profile model remains missing) |
...
| **Configuration Groups** | `groups { name { ... } }; apply-groups name`; `apply-groups-except name` | Template inheritance for config reuse. Apply common settings to multiple stanzas without duplication. | Medium | Done (general-purpose groups/apply-groups with inheritance priority, `${node}` variable support, `CompileConfigForNode()` for HA per-node config). `apply-groups-except` suppresses inheritance from the named group at the stanza that carries it **and below**, per-group and per-destination (a wildcard group key is excluded only at the containers that carry the statement); a name matching no defined group is a no-op. A group key of the form `<pattern>` is glob-matched against the instance name (`<ge-*>`, `<*-0/0/0>`), with `*` crossing `/`; a pattern that matches nothing applies nothing (#9423 — it was previously adopted as a phantom instance literally named after the pattern). **This line previously claimed `apply-groups-except` as Done while nothing consulted it (#9422) — the statement was accepted by all four config channels and the excluded group was inherited anyway.** |
...
| **Rescue Configuration** | `request system configuration rescue save` | Saved fallback configuration that can be loaded on boot if active config fails | Low | **Done.** `request system configuration rescue save`/`delete` are implemented — `SaveRescueConfig`/`DeleteRescueConfig` persist the active config text to `rescue.conf` (owner-only 0600, #4056; DurableState #1894) in `pkg/configstore/store_persist.go`, wired through `pkg/cli/cli_request.go`. Since #8597 K47 the REMOTE `cli` implements both too — `cmd/cli` `handleRequestSystemConfiguration` issues the `rescue-save` / `rescue-delete` SystemAction verbs, which the remote dispatcher previously answered with `unknown request system command: configuration` although the SSOT command tree advertises them. |
...
| Feature | Junos Config Path | Description | Priority | Status |
|---------|-------------------|-------------|----------|--------|
| **Link Aggregation (LAG/ae)** | `interfaces ae0 ...; interfaces ge-0/0/0 gigether-options 802.3ad ae0` | Bundle physical links into aggregate ethernet for bandwidth and redundancy. Different from reth. | Medium | **Config-only (#6544)** — was listed "Done (LACP/802.3ad parsing + bond/netdev generation + member enslaving)"; that was wrong. `aggregated-ether-options` compiles to `AggregatedEtherOpts` and `gigether-options 802.3ad <ae>` to `LAGParent`, and NOTHING reads either field. Measured on the compiled config: an `ae0` with `lacp active`, `periodic fast`, `minimum-links 2` and two bound members yields ZERO bond models from `buildFabricBondModels` (`pkg/dataplane/compiler_iface.go`), so no `.netdev` is written, no member gets `Bond=`, no bond device is created, no member is enslaved, no LACP runs and `minimum-links` is not honoured. The bond machinery is NOT missing — `pkg/networkd` `generateNetdev` already emits `Mode=802.3ad` + `LACPTransmitRate` + `TransmitHashPolicy` + `MinLinks`, and `pkg/routing/bond.go` reconciles kernel bonds — but BOTH are reached only from `fabric-options member-interfaces`, which hard-codes `active-backup`. Wiring `ae` into them is the remaining work, tracked in the parity matrix (`docs/vsrx-gaps.md`, "Interface Redundancy (LAG)"). Carries the accepted-only advisory (`validateLinkAggregationWarnings`, the #2078/#4231/#5804 doctrine) so the commit no longer reports a working aggregate; a hard reject was rejected as the posture because inert `ae` config is not dangerous and rejecting it would brick an already-persisted config (#1960). |
...
15. **Link Aggregation (LAG)** - **Config-only (#6544)** — `ae` / `802.3ad` is schema-advertised and commits clean but creates no bond, enslaves no member, runs no LACP and does not honour `minimum-links`; it carries an accepted-only advisory. Previously listed here as Done, which it never was. Feature work tracked in `docs/vsrx-gaps.md`.
...
- **#8621 correction to the deferral's stated reason.** That deferral records
  ...
  `arp_fwd_proxy` returns 0 on `rt->dst.dev == dev` before ever reading it. The
  same-L2 case is now served by the userspace responder instead, which is
  per-address by construction. So the deferral's blocker is gone -- narrowing
  the sysctl can no longer re-break #2160, because the sysctl was never fixing
  it. Whether to narrow is still open; the reason it could not be is not.
- **Why there is no IPv6 responder, and it is not an oversight.** v6 proxy NDP
  has none of this: `ndisc_recv_ns` gates on forwarding + `proxy_ndp` + a bare
  `pneigh_lookup` (`pndisc_is_router`) with NO route lookup, so there is nothing
  corresponding to `rt->dst.dev != dev` and the kernel answers a same-subnet v6
  target correctly. A v6 responder would duplicate a working kernel path, and
  two responders for one family can disagree while only one is ownership-gated.
