...
| **ICMP Session Sync** | `security flow sync-icmp-session` | Synchronize ICMP sessions between HA cluster nodes | Low | **Config-only (#4231)** — schema-typed leaf; accepted-but-not-enforced. The userspace dataplane does NOT sync ICMP sessions to the HA peer, so ICMP flows are not replicated and do not survive a failover. Commit emits a STRONG HA-specific advisory (previously a silent drop that could mislead an operator into believing ICMP sessions sync). |
