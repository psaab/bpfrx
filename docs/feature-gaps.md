...
| **TCP Initial Timeout** | `security flow tcp-session initial-timeout N` | Idle timeout for a TCP session whose three-way handshake has not completed (the half-open / SYN-flood bounding control) | Medium | **Config-only (#6539)** — modeled, parsed and committable, but there is NO wire field for it: `buildFlowSnapshot` lowers `established-timeout` alone. The dataplane bounds the state on a fixed 20s window (`DEFAULT_TCP_OPENING_TIMEOUT_NS`, #3152). The one half-open window an operator CAN move today is the per-zone `screen ... syn-flood timeout` override (#3527). Accepted-but-not-enforced; commit emits an advisory and all three `show` surfaces annotate the value. |
| **TCP Closing Timeout** | `security flow tcp-session closing-timeout N` | Idle timeout for a TCP session that has begun closing | Medium | **Config-only (#6539)** — no wire field. The dataplane reaps a graceful FIN close on a fixed 30s window and a RST abort on 2s (`TCP_CLOSING_TIMEOUT_NS` / `TCP_RST_TIMEOUT_NS`, #3046). Accepted-but-not-enforced; commit emits an advisory and all three `show` surfaces annotate the value. |
| **TCP Time-Wait Timeout** | `security flow tcp-session time-wait-timeout N` | Idle timeout for a session in TIME_WAIT | Low | **Config-only (#6539)** — no wire field AND no state to attach one to: `session_timeout_ns` splits a TCP close only into RST vs FIN, so there is no TIME_WAIT window distinct from `closing-timeout`'s. Enforcing this leaf needs a close-state split in the Rust session machine first. Accepted-but-not-enforced; commit emits an advisory and all three `show` surfaces annotate the value. |
