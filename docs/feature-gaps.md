...
| **Deterministic NAT (Port Block Allocation)** | `security nat source pool ... port deterministic ...` | Predictable port mapping for logging compliance. Each source gets fixed port block. | Low | **Config-accepted, runtime-inert on userspace (#4559)** -- typed + validated at commit (`compiler_nat.go`), but the block allocator was only ever wired into the eBPF plane (`74e1d17`, `439cd3f`), which was retired in #1373/#1476 and never ported to the userspace dataplane (`nat_source.go` has 0 refs). A deterministic pool commits clean and SILENTLY falls back to round-robin/sticky SNAT; commit now emits an accepted-but-inert advisory (ValidateConfig, #4559). Full block-allocation enforcement in `userspace-dp` remains a follow-up (#4559). |
