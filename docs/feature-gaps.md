...
| **Policy Rematch** | `security policies policy-rematch` | Re-evaluate existing sessions when policy changes | Medium | **Partial (#4234).** The Junos-DEFAULT deletion-clear is ENFORCED: a session admitted by a policy that a commit DELETES is invalidated immediately at commit (`pkg/daemon/daemon_policy_invalidate.go`, keyed on the session `policy_id` #3056, propagated to the HA peer via the #2468 delete-sync), independent of the knob. The `policy-rematch` knob itself (re-evaluating an in-progress session against a MODIFIED — not deleted — policy) is still accepted-only + advisory (#4233); that modified-policy re-eval is the needs-research half of #4234. |
