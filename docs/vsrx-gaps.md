# vSRX Feature Gaps — bpfrx vs Juniper vSRX
...
| **Advanced Policy-Based Routing (APBR)** | Route traffic based on detected application identity (not just L3/L4 match but L7 app detection) | Complex | No — bpfrx has filter-based PBR but not app-aware routing |
...
| **SecIntel Threat Feeds** | Curated threat intelligence feeds (C&C, attacker IPs, malicious URLs) | Medium | No — bpfrx has dynamic address feeds but not Juniper SecIntel integration |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **Twice NAT** | Simultaneous source and destination NAT in a single rule | Medium | No — bpfrx has separate SNAT/DNAT but not combined twice-NAT rules |
...
| **NAT Rule Ordering** | Explicit rule ordering with priority numbers | Simple | No — bpfrx uses implicit ordering |
...
| **DS-Lite Concentrator** | IPv4-in-IPv6 softwire for carrier-grade NAT | Complex | No — bpfrx has NAT64 but not DS-Lite |
...
| **BFD** | Bidirectional Forwarding Detection for fast failure detection | Medium | No — FRR supports BFD but bpfrx doesn't configure it |
| **Routing Policy (import/export)** | Full policy-options with route-maps, prefix-lists, AS-path filters for BGP/OSPF/IS-IS | Medium | Partial — bpfrx has basic export/redistribute but not full policy-language |
| **Route Flap Damping** | BGP route stability controls | Simple | No |
| **Graceful Restart (GR)** | Non-stop routing during control plane restart | Medium | No — FRR supports GR but bpfrx doesn't configure it |
| **OSPFv3** | OSPF for IPv6 | Medium | No — bpfrx configures OSPF (v2) but not OSPFv3 |
...
| **IPsec Dead Peer Detection (DPD)** | Configurable DPD intervals and actions | Simple | No — strongSwan has DPD but bpfrx may not configure it |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **Chassis Cluster** | Active/passive or active/active HA with two devices as one logical unit | Complex | No — bpfrx has VRRP but not full chassis cluster |
...
| **IP Monitoring for Failover** | Track remote IP reachability to trigger failover | Medium | No — bpfrx has RPM probes but not HA failover triggers |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **NETCONF/YANG** | Standards-based configuration management protocol | Complex | No — bpfrx has gRPC/REST but not NETCONF |
| **J-Web GUI** | Full web-based management interface with dashboard, wizards, monitoring | Complex | No — bpfrx has basic REST API |
| **Security Director** | Centralized multi-device policy management (Junos Space) | Complex | No (N/A — cloud/controller feature) |
| **SNMP Traps/Notifications** | SNMP v2c/v3 trap sending on events | Medium | No — bpfrx has SNMP agent (GET) but no traps |
| **SNMP v3** | SNMPv3 with authentication and encryption | Medium | No — bpfrx has basic SNMP v2c only |
...
| **Event Policies** | Automatic actions triggered by system events (syslog patterns) | Medium | Partial — bpfrx has `pkg/eventengine` but unclear feature parity |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **Traffic Policing** | Token-bucket rate limiting on interfaces (single-rate, two-color, three-color) | Medium | No — bpfrx has screen rate-limiting but not interface policing |
...
| **Behavior Aggregate Classification** | Classify incoming traffic based on DSCP/802.1p into forwarding classes | Simple | No — bpfrx has DSCP matching in filters but not full BA classification |
| **Rewrite Rules** | Rewrite DSCP/802.1p markers on egress | Simple | Partial — bpfrx has forwarding-class DSCP rewrite in filters |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **Structured Syslog** | Machine-parseable syslog with key-value pairs (Junos structured format) | Medium | No — bpfrx has syslog but not structured format |
...
| Feature | Description | Complexity | Parse-only? |
|---------|-------------|-----------|-------------|
| **Unified Policies** | Single policy combining L3/L4 + L7 app + URL category + user identity | Complex | No — bpfrx has zone-based policies but not unified (needs AppID) |
...
| **System Alarms on License Expiry** | License monitoring and alarming | Simple | No — bpfrx has no licensing |
...
These features exist in the bpfrx parser/AST but have NO runtime effect:
