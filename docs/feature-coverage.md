...
| Screen/IDS (16 checks) | Yes; userspace SYN-cookie runtime is wired. **Unresolved profile reference visibility (#5806):** strict commit REJECTS a zone that references an undefined `screen ids-option` profile, but tolerant startup/recovery of an older or externally modified `active.json`, HA config-sync from a schema-skewed peer, and rolling-upgrade intervals all downgrade that to a warning — and the dataplane USED to enforce NONE of that zone's screen checks while the active config still claimed a screen was attached. **#7168 settled that posture** — see the enforcement paragraph below. That state is now reported on three surfaces instead of a single rate-limited runtime WARN (#6860, FIXED: the WARN previously could not fire when NO profile resolved at all — `stage_screen_check` gated on `has_profiles()`, which consults only the RESOLVED map, so the all-or-nothing tolerant-load shape returned before either missing-profile branch. The gate now asks `has_screen_state()` — is there something to ENFORCE *or* something to REPORT — and the runtime WARN fires in the one configuration it was written for. `has_profiles()` keeps its meaning and its other call site: `stage_screen_syn_cookie_ack_on_session_miss` only enforces and emits no diagnostic, so an unresolved reference gives it nothing to do and widening that gate would run the stage on every packet for no signal. Partial resolution was never affected): the Prometheus gauge `xpf_screen_unresolved_profile_zones{zone,profile}` (config-derived, so it is emitted even on a config-only / degraded boot, and present ONLY while a reference is unresolved — `max_over_time(...)` alerts on any zone ever left unscreened), plus an `Unresolved screen profile references:` block at the top of `show security screen` in BOTH the local CLI and the gRPC renderer. All three derive from ONE exported builder, `dpuserspace.ScreenMissingProfileRefs` — the same function whose output is published to the helper as `ConfigSnapshot.ScreenMissingProfiles` and drives the dataplane WARN. Routing every surface through it is the CONTRACT, not a convenience — stated with its condition: WHENEVER a snapshot has been published for the config being rendered, that snapshot's `ScreenMissingProfiles` and every surface above are the same function of the same input, so no surface can name a different set than the dataplane was told about. That identity is Go-struct to Go-struct; the dataplane is the Rust helper and reads JSON, so struct → wire → decoder is a SECOND hop, bound separately by `TestScreenMissingProfilesPublishedToSnapshot`, which marshals the snapshot and pins the wire key `screen_missing_profile_zones` with its `zone`/`profile` elements. The unqualified form ("impossible to report a different set") is false when NO snapshot has been published — the config-only / degraded boot these surfaces exist to cover — because there is then no told-about set to disagree with, and what the surfaces report is the set the dataplane WOULD be told on the next publish. The enforcement disposition is carried in the metric's HELP text and in ONE status line — not as a label — because it is a global statement about the implementation, identical for every zone, and a prose label value would add unbounded cardinality the day it varies. Its wording (`dpuserspace.ScreenUnresolvedDisposition`) states what #7168 actually does: *the profile reference does not resolve, so the dataplane enforces a substituted conservative default for this zone — the threshold-free malformed-packet checks only, with NO flood, scan or session-limit thresholds synthesised; policy evaluation is unaffected. The zone is protected but its configured profile is NOT in effect, so the reference still needs fixing.* It is NOT "forwarded unscreened", which reads as a permit and would suggest the firewall is passing traffic it would otherwise deny — zone policy still evaluates the packet normally. That string carries literal `#5806` AND `#7168` anchors because it asserts a decision made in the RUST runtime (`screen/mod.rs`'s `missing_profile_verdict` on the `None` branch) that the Go control plane cannot derive; both are required so a reader grepping from either the original issue or the settled one lands here. **Posture, SETTLED in #7168:** neither endpoint of the old fork was right. Failing OPEN silently removes a configured security control; failing CLOSED turns a config-reference typo into a per-zone outage on the one path whose reason to exist is #1960 no-brick, i.e. it converts a security gap into an availability incident at exactly the moment the tolerant path is load-bearing. Instead the zone is enforced against `ScreenProfile::conservative_default()`: the threshold-free malformed-packet subset (`land`, `teardrop`, `winnuke`, `syn-fin`, `no-flag`, `fin-no-ack`, `syn-frag`, `ping-death`, `source-route-option`) and NO rate checks. The line is drawn by whether a value must be GUESSED — those checks take no operator threshold and drop only packets invalid on their face, whereas a guessed flood/scan/session-limit threshold is site-specific and would itself be the outage. `icmp-fragment` is deliberately excluded despite being a threshold-free bool: a large ping legitimately fragments, so it could black-hole merely atypical traffic. Only STATELESS checks run, and that is structural rather than remembered — the rate checks need per-zone tracker state that by definition does not exist for an unresolved zone. Both the flow-present and flowless `None` branches route through it; a zone with NO screen configured still passes silently; drops are counted separately (`substituted_default_drops`) because a drop there means a broken reference is being MASKED and still needs fixing; and recovery is automatic, since the substitution is a property of the resolved map. **Third state — DEFINED but enforcing nothing (#7059/#7060):** "defined" is not "enforced". `buildScreenSnapshots` publishes a profile only when at least one check is enabled, so a zone whose profile is defined but enables ZERO checks publishes no snapshot, `ScreenState::zones` has no entry, and the zone's CONFIGURED profile enforces nothing — while every surface above stayed silent, because the reference *resolves*. Since #7888 the dataplane substitutes the same conservative default it uses for an unresolved reference, so the zone is protected even though the profile the operator wrote is not in effect; before #7888 it got a bare `Pass`. That state is strictly MORE reachable than the unresolved one it sat beside: `set security screen ids-option p alarm-without-drop` and nothing else passes STRICT commit with zero warnings (measured), whereas a dangling reference is strict-REJECTED and reachable only through the tolerant paths. It is now reported by its own distinct surfaces — the gauge `xpf_screen_inert_profile_zones{zone,profile}`, a `Screen profiles enforcing nothing:` block, and `dpuserspace.ScreenInertDisposition` — deliberately SEPARATE from the unresolved ones rather than folded into them, because `xpf_screen_unresolved_profile_zones`' HELP says "not defined" and widening it would make a shipped series' documented meaning false and silently change what existing alerts match. The two dispositions share a tail, the half of the sentence true of both, and its ownership has tracked one fact moving. #7059 shared it (`screenNoEnforcementTail`) because both states had the identical consequence. **#7168 split it**, because an unresolved reference became enforced against the substituted default, so "no screen checks are applied" was false for it and still true for the inert state. **#7888 re-shared it** (`screenSubstitutedDefaultTail`) after settling the same posture for the inert state: two configurations with the identical consequence must not get different protection, and the one that looks MORE correct to an operator must not get LESS. `TestScreenUnresolvedDispositionHasOneSource` still requires exactly ONE occurrence of that literal across pkg/ and cmd/; its self-check now anchors on BOTH constants, and it additionally asserts the two dispositions have not collapsed into one message — sharing the CONSEQUENCE must not erase the diagnosis, since "define the profile" and "add a check to it" are different remedies. `ScreenUnresolvedDisposition`'s value is byte-unchanged by that re-sourcing. The SSOT is `dpuserspace.ScreenInertProfileRefs`, which asks `buildScreenSnapshots` what it actually PUBLISHED rather than re-deriving the emit gate — a copy of the gate could disagree with the dataplane, a call to the publisher cannot. #7060 additionally carries both blocks into the two per-profile renderers (`show security screen ids-option <p>` and `... detail`), which print their own empty-inventory / `not found` line and previously answered a stranded zone with silence; the ordering contract `CheckScreenDiagnosticRenderOrderBefore` covers that second anchor so no third hand-written ordering assertion exists. NOT reported: a zone with no screen configured at all (nothing was asked for), which is why the three-state test carries that row. **The dataplane's runtime WARN covers this state as of #7888.** It used to warn "references undefined screen profile" — factually wrong here, and word-for-word the strict commit-time error in `compiler_validate_strict_screen.go` for a genuinely missing profile, so an operator went looking for an `ids-option` stanza that is present with no commit-time evidence to reconcile against. The inert set is now carried on the wire as `screen_inert_profile_zones`: an additive SIBLING of `screen_missing_profile_zones`, `omitempty` on the Go side and `#[serde(default)]` on the Rust side, so both skew directions degrade to the pre-#7888 behaviour (old helper ignores the field; old Go binary leaves the set empty — inert zones `Pass`, exactly as before). `ScreenState` holds it in a SECOND map, `inert_profile_refs`, deliberately NOT folded into `missing_profile_refs`: one map carrying both states would make the two WARN texts undecidable at the point they are emitted, which is this very defect one layer down. `unresolved_screen_kind` reads the three states off membership, with the legitimate silent `Pass` being "in neither map", and the inert WARN leads with IS DEFINED and names the actual cause (a modifier is not a check) rather than repeating the undefined wording. |
| Firewall filters + policers | Filters yes; three-color policers admitted for the reviewed color-blind `then discard` slice; broader color-aware and non-drop action work is tracked as production hardening. **Dangling `then policer` reference (#6540):** a term naming a policer defined under neither `firewall policer` nor `firewall three-color-policer` used to compile cleanly and forward UNPOLICED — the reference resolved to `None` and the meter no-opped, with no error, warning or counter. Strict commit rejects such a reference (#2217 Finding A); the Rust helper now also rejects the snapshot (`SnapshotIntegrityError::MissingPolicerRef`), closing the lenient boot / HA-peer-sync route that was the only remaining gate. The predicate is DEFINEDNESS, not presence in the compiled runtime map: #4514 deliberately skips a degenerate zero-rate meter-only policer, which is defined and has no action to enforce, so keying the rejection on the map would refuse a config that boots today. |
