...
ROUND 3: PR ready to merge

H1: CLOSED — the allowlist is now authoritative. `UserspaceBoundLinuxInterfaces()` derives the Linux-name set from the compiled userspace binding scope (`pkg/dataplane/userspace/snapshot.go:77-137`); startup and reconcile both pass that list into D3 (`pkg/daemon/daemon.go:454-477`, `pkg/daemon/daemon.go:2301-2312`); and `applyRSSIndirection()` / restore iterate only `allowed`, with empty allowlists as explicit no-ops (`pkg/daemon/rss_indirection.go:118-123`, `pkg/daemon/rss_indirection.go:138-206`). In the common mixed-NIC case of a sibling mlx5 PF present in sysfs but not zoned/bound to userspace-dp, that PF is excluded and the test pins it (`pkg/daemon/rss_indirection_test.go:376-444`). Standalone/non-HA is fine because fabric snapshots are nil without cluster config (`pkg/dataplane/userspace/snapshot.go:192-195`) and the standalone allowlist test still returns only zoned dataplane links (`pkg/dataplane/userspace/snapshot_allowlist_test.go:15-46`).

H2: CLOSED — local CLI bare `commit`, `commit comment`, and `commit confirmed N` all route through `c.commitApply(compiled)` (`pkg/cli/cli_config.go:185-251`), `commitApply()` prefers `applyConfigFn` (`pkg/cli/cli_config.go:12-32`), and the daemon wires `d.applyConfig` into the in-process CLI (`pkg/daemon/daemon.go:1082-1090`). Auto-rollback expiry is covered separately: the CLI registers a central rollback handler that also prefers `applyConfigFn` (`pkg/cli/cli.go:569-583`), and the timer invokes that handler on expiry (`pkg/configstore/store.go:843-884`). The only bare-`commit` branch that skips `commitApply` is confirm-pending, which just calls `ConfirmCommit()` and does not apply a new config (`pkg/cli/cli_config.go:233-239`).

M: CLOSED — the doc now tells operators exactly how to disable D3, that commit-time reconcile applies regardless of gRPC/HTTP/local CLI, that `disable` restores the driver's default table, and what to do before stopping the daemon if D3 misbehaves (`docs/785-d3-validation.md:36-79`).

LOW — `TestReapplyRSSIndirection_EndToEndWritesWeights` proves only `reapplyRSSIndirectionWith() -> ethtool -X ... weight ...` (`pkg/daemon/linksetup.go:124-132`, `pkg/daemon/rss_indirection_test.go:446-476`); it does not exercise `Daemon.applyConfig()` step 21 or a real CLI/API commit (`pkg/daemon/daemon.go:2292-2312`). The commit-path and reapply-path checks remain split (`pkg/cli/cli_commit_test.go:14-60`).
