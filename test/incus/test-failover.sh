...
#   5. Verify fw0 comes back and preempts to primary
#
# Usage:
...
# ── Phase 4: Wait for fw0 to come back and preempt ──────────────────

info "Waiting for fw0 to reboot and preempt back to primary (max ${REBOOT_WAIT}s)"
...
# Give it a moment to preempt
sleep 10

# Verify fw0 is primary again
if incus exec bpfrx-fw0 -- cli -c 'show chassis cluster status' 2>/dev/null | grep -q "node0.*primary"; then
	pass "fw0 preempted back to primary"
else
	fail "fw0 did not preempt back to primary"
fi

# Verify iperf3 still running after failback
if incus exec cluster-lan-host -- pgrep iperf3 &>/dev/null; then
	pass "iperf3 survived failback (fw0 preemption)"
else
	# Could have finished naturally — check if it completed successfully
	if incus exec cluster-lan-host -- grep -q "iperf Done" /tmp/iperf3-failover.log 2>/dev/null; then
		pass "iperf3 completed successfully (finished before failback check)"
	else
		fail "iperf3 DIED during failback"
	fi
fi
