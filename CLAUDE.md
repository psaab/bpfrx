...
- **Flow**: TCP MSS clamping (ingress XDP + egress TC on the legacy path, plus userspace handling where admitted), ALG control, allow-dns-reply, allow-embedded-icmp, configurable timeouts (per-application inactivity), strict-syn-check-style session-miss guard (a bare TCP RST/FIN with no SYN never seeds a session — transit dispositions DROP the packet, #4400; the host-inbound LocalDelivery path declines to CACHE it but still delivers it to the local stack so a peer teardown for a firewall-originated flow is not lost, #4487), firewall filters (port ranges, hit counters, logging, forwarding-class DSCP rewrite, DSCP action)
