...
- **Screen/IDS**: 12 checks (land, syn-flood, ping-death, teardrop, rate-limiting, icmp-fragment, ip-sweep, syn-frag, syn-fin, no-flag, winnuke, fin-no-ack), SYN cookie flood protection (XDP-generated SYN-ACK cookies with source validation), SYN-flood sub-thresholds (#3315: per-source/per-destination caps on a no-eviction count-min sketch + log-only alarm-threshold; `timeout` ENFORCED in #3527 as a per-zone override of the half-open session window `tcp_opening_ns`)
