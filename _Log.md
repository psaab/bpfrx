...
- **Timestamp**: 2026-06-19
- **Action**: #2053 /research plan-draft (companion-free half). Drafted docs/research/2053-secret-marshal-redaction/plan.md (11 sections, status DRAFT) + hostile claude-smr-plan-r1.md. KEY CORRECTION to the issue premise: the issue claims "no live leak today" (ExportJSON has no prod callers) — but GET /api/v1/config (pkg/api/server.go:184 -> configHandler pkg/api/config.go:14 -> writeJSON json.Encode) IS a registered production REST route that marshals the full compiled *config.Config including every secret in plaintext. Live leak (loopback default; non-loopback via web-management https interface). Enumerated 16 secret fields across types_security/routing/system/interfaces.go. Round-trip is SAFE: nothing unmarshals a compiled *config.Config (db.go persists *ConfigTree AST, not the compiled struct), so a redacting Secret wrapper (option A) cannot break any consumer. Recommendation PLAN-READY. Branch research/2053-secret-marshal-redaction.
- **File(s)**: docs/research/2053-secret-marshal-redaction/plan.md, docs/research/2053-secret-marshal-redaction/claude-smr-plan-r1.md, _Log.md
