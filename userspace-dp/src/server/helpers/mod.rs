...
// into cohesive submodules (#6234): the former 1.4k-line grab-bag became
// an ownership boundary for status projection, HA session-sync
// reconstruction, binding/queue planning, and state persistence. This
// file is now a narrow re-export facade only — every helper lives in one
// of the submodules below and is reached by call sites through the same
// `server::helpers::<name>` paths as before.
//
...
//
// Pure relocation. Bodies byte-for-byte identical to the pre-split source.

mod session_sync;

pub(crate) use session_sync::*;

// Remaining inline helpers (status projection, planning, persistence) are
// extracted into sibling submodules in the follow-on #6234 commits; until
// then they keep the original crate-root glob + hashing imports.
use super::super::*;
