...
fn live_flow_cache_callsite_nonsampled_does_not_reserve_full_queue_6304() {
    ...
    // in place (TTL decrement + checksum delta). Handing the heap `frame` in here
    // instead would decouple the mirror clone from the rewrite and make "the clone
    // carries the PRE-rewrite frame" true for free — the capture-before-rewrite
    // ordering at flow_cache_hit.rs would be unbound. `frame` stays pristine as
...
/// `mirror_sample_counter = 1` at `rate = 2`, so `mirror_sample_allows` is FALSE
/// and `MirrorSampleAdmission::NotSampled` short-circuits — everything
/// downstream of `Sampled(..)` at the live call site is left unbound by it. Two
/// mutations confirmed that firsthand, both leaving `mirror/resolver.rs`
...
/// observation. The non-sampled test above asserts the drop counters are ZERO,
/// so nothing in it depends on the target being present, cross-worker, or full;
/// a refactor of `MirrorTargetMap` keying or `resolve_tx_binding_ifindex` would
/// leave it passing and binding nothing. Asserting a NON-zero
/// `mirror_drops_queue_full_cross_worker` here fails loudly if the fixture ever
/// stops reaching a genuinely full cross-worker target.
#[test]
fn live_flow_cache_callsite_selected_full_queue_advances_sampler_6304() {
    ...
    // rate = 2 with counter = 0 -> `mirror_sample_allows` is TRUE: this packet IS
    // selected, so it reaches the admission on the full queue.
    let mut mirror_sample_counter: u64 = 0;
    ...
    // --- WIRING, asserted rather than merely observed: a NON-zero cross-worker
    // full-queue drop can only be produced by a mirror target that resolved, is
    // on another worker, and is at its cap. A `NoBinding` short-circuit would
    // read 0 here.
    assert_eq!(
        ingress_live.mirror_drops_queue_full.load(Ordering::Relaxed),
        1,
    ...
    assert_eq!(
        ingress_live.mirrored_packets.load(Ordering::Relaxed),
        0,
...
/// BOTH tests above leave untouched — the non-sampled one short-circuits before
/// admission, and the full-queue one only ever reaches `Sampled(Err)`. Mutating
/// `Sampled(Ok(_)) => None` (every selected packet silently drops its clone;
/// port mirroring stops delivering on this path) is green under both.
///
/// Here the cross-worker target has room, so the selected packet must actually
/// land a full-L2 clone in the target's queue and be counted as mirrored.
#[test]
fn live_flow_cache_callsite_selected_admitted_clone_reaches_target_6304() {
    ...
    // A DIFFERENT binding is the mirror target, and its clone queue has ROOM
    // (no `set_max_pending_tx`, no pre-fill) — the admission must succeed.
    let target_live = Arc::new(BindingLiveState::new());
    let mut mirror_targets = MirrorTargetMap::default();
    mirror_targets.insert(
        &BindingIdentity {
    ...
    // rate = 2 with counter = 0 -> selected.
    let mut mirror_sample_counter: u64 = 0;
    ...
    // The clone is a real full-L2 copy on the TARGET binding's queue, flagged as
    // a mirror clone and addressed to the mirror output interface.
    ...
    assert_eq!(
        clone.egress_ifindex, MIRROR_OUT_IFINDEX,
        "the clone egresses the mirror output interface"
    );
    assert_eq!(
        clone.bytes, frame,
        "the clone carries the full L2 frame verbatim"
    );
    assert!(
        queued.is_empty(),
