...
fn build_worker_cos_statuses_owner_profile_only_surfaces_on_unambiguous_owner_local_exact_queue() {
    ...
    // (the owner-local exact queue in this fixture).
    {
        ...
        let q4 = runtime
            ...
            .find(|q| q.queue_id() == 4)
            .unwrap();
        q4.telemetry.owner_profile.drain_latency_hist[2].store(9, Ordering::Relaxed);
        q4.telemetry
            .owner_profile
            .drain_invocations
...
fn build_worker_cos_statuses_surfaces_distinct_per_queue_drain_telemetry() {
    ...
    // Queue 4: "slow drain" profile — landings in high bucket.
    {
        ...
        q.telemetry.owner_profile.drain_latency_hist[12].store(7, Ordering::Relaxed);
        q.telemetry
            .owner_profile
            .drain_invocations
    ...
    // to a single profile fails the per-queue distinctness check.
    {
        ...
        q.telemetry.owner_profile.drain_latency_hist[2].store(23, Ordering::Relaxed);
        q.telemetry
            .owner_profile
            .drain_invocations
