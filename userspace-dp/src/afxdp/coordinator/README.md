...
| `wg_control/` | WG datapath control-thread BODY (one supervised aux thread per `mode == "wireguard"` endpoint, kernel UDP socket + wgN TUN). `mod.rs` holds the thread entry (`wg_control_loop`) and the orchestrating `run_wg_control_loop` (socket RX burst → TUN-read burst → 1s timer arm → idle poll(2)); the fused layers are split into `mtu.rs` (pad-aware encapped-size formula + per-peer outer-MTU guard), `sock.rs` (dual-stack bind, v4-mapped send shim, #2317 outer-TOS cmsg codec, poll(2) wait layer), `attempt.rs` (#1888 S5 handshake attempt machine + keepalive emit/pace), `dispatch.rs` (inbound type-byte dispatch with the auth-before-roam `InboundOutcome` contract + the TUN-read encap-and-send), and `policy_gate.rs` (#5618 inner-ingress FORWARD zone-policy authority: the decapped plaintext is evaluated as ingress on the tunnel's LOGICAL interface zone + routing instance against the routed egress interface's zone, and a non-permit verdict DROPS it before the TUN write — closing the inter-zone bypass where the kernel FIB decided everything. A packet with NO parseable 5-tuple is adjudicated on its L3 identity via the shared `evaluate_policy_result_l3_aware(.., l4_present = false)` path, never delegated — otherwise an authenticated peer bypasses the deny by picking any IP protocol other than TCP/UDP/ICMP-query, or by fragmenting; a discard route, an IPv6 extension chain that is OVER-LIMIT or TRUNCATED, a packet declaring TCP/UDP with no readable L4 header (non-first fragments exempt), and a non-IP payload DROP as `inner_forward_drops` — narrow on purpose, since an IPv6 `NoNextHeader` terminal is adjudicated flowlessly (stamped protocol 59, not the base Next Header byte) rather than dropped, while a TRUNCATED chain leaves only an extension-header number to adjudicate on and so cannot match a protocol- or port-keyed rule, which mainline handles by dropping the shape at XDP ingress; a peer-selected unzoned EGRESS is adjudicated with `to_zone = 0` (#3110 default-action fall-through) rather than delegated; and the slow-path-ELIGIBLE dispositions `NoRoute`/`NextTableUnsupported`, host-bound `LocalDelivery`, plus a unit carrying a `then routing-instance` input filter (FBF moves the egress the to-zone is derived from, and the gate evaluates no filters) delegate for mainline parity, COUNTED via `inner_policy_unadjudicated`). The loop takes a `RuntimeViewReader` and refreshes its `Arc<ForwardingState>` ONCE per iteration (the #1881 D.1 pattern, pinned by the #6592 runtime-view canary) so a burst is never split across two policy generations. (#6438 split; `pad_to_16` is SSOT in `afxdp/wg/mod.rs`.) |
