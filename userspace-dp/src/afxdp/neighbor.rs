...
pub(super) fn add_kernel_neighbor_probe(ifindex: i32, ip: IpAddr) {
...
pub(super) fn insert_vlan_tag(frame: &mut Vec<u8>, vlan_id: u16) {
    if frame.len() < 14 {
        return;
    }
...
pub(super) fn send_raw_frame(ifindex: i32, frame: &[u8]) {
    if frame.len() < 14 {
        return;
    }
