...
| `nat.rs` | NAT44 | Source / destination / static NAT decisions. `NatDecision` carries `rewrite_src` and `rewrite_dst` Options the TX path consumes. **#2394:** the DNAT table (`nat/destination.rs`) matches a rule on `(protocol, dst_ip, dst_port)` AND its `match source-address` constraint — a source-scoped DNAT (`DnatEntry.source_v4/source_v6`) fires only for packets whose source IP falls in a configured prefix; an entry with no source prefixes matches any source (unscoped behavior). Before #2394 the source constraint was dropped at the Go->Rust snapshot boundary, so a source-scoped DNAT became destination-only and published the internal service to every source (fail-open). |
