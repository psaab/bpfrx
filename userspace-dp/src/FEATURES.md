...
|------|-------|--------------|
| `screen.rs` | screen | Pre-session attack-protection checks (land, TCP SYN+FIN, no-flag, FIN-without-ACK, ICMP frag, plus rate-limits). Mirrors `bpf/xdp/xdp_screen.c`. Also contains the #1374 userspace SYN-cookie runtime: deterministic cookie mint/validate, fixed-size validated-client admission table, session-miss ACK validation, bounded SYN-ACK/RST reply enqueue, snapshot-published key derived from cluster-synced root encrypted-password material, and per-binding sent/valid/invalid/bypass/budget counters. The Go `syn-cookie` capability gate is open; missing secret material fails closed and remaining #1374 work is live HA/flood evidence before BPF source removal. |
