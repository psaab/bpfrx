...
mod tests {
    ...
    #[test]
    fn rewrite_forwarded_frame_in_place_trims_padding_from_meta_pkt_len() {
        let src_ip = "2001:559:8585:ef00::102".parse::<Ipv6Addr>().unwrap();
        let dst_ip = "2001:559:8585:80::200".parse::<Ipv6Addr>().unwrap();
        ...
        .expect("rewrite in place");
        assert_eq!(frame_len, 14 + 60);
        let out = area.slice(0, frame_len as usize).expect("rewritten frame");
        assert_eq!(tcp_ports_ipv6(&out[14..]), (src_port, dst_port));
    ...
    #[test]
    fn build_forwarded_frame_into_trims_padding_from_meta_pkt_len() {
        let src_ip = Ipv4Addr::new(10, 0, 61, 102);
        let dst_ip = Ipv4Addr::new(172, 16, 80, 200);
        let src_port = 42424u16;
        let dst_port = 5201u16;
        let mut frame = Vec::new();
        write_eth_header(
            &mut frame,
        ...
        assert!(tcp_checksum_ok_ipv4(&out[14..frame_len]));
    }
