...
fn synced_session_rejects_unresolved_ipv6_ext_protocol_6923() {
    ...
    for protocol in &traversable {
        let err = build_synced_session_key(&sync_req(v6, *protocol, "2001:db8::11", "2001:db8::22"), 0)
            .expect_err(&format!(
    ...
    for protocol in [6u8, 17, 50, 58, 59, 47] {
        let key =
            build_synced_session_key(&sync_req(v6, protocol, "2001:db8::11", "2001:db8::22"), 0)
                .unwrap_or_else(|e| {
