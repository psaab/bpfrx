...
// The destination leaf gets groupReplace for the same reason the source one
// does, and this pins it. Its value list packs `port`, so apply-groups token
// UNION corrupts the ADDRESS outright rather than merely over-collecting:
// inheriting `address 10.0.0.2/32 port 8080;` over an inline
// `address 10.0.0.1/32 port 80;` compiled the address as "8080".
func TestDestinationNATPoolAddressGroupOverride8800(t *testing.T) {
	...
	if addr != "10.0.0.1/32" || port != 80 {
		t.Errorf("apply-groups over a destination NAT pool address: got addr=%q port=%d, want addr=\"10.0.0.1/32\" port=80\n"+
			"The inline statement must OVERRIDE the inherited one. addr=\"8080\" means "+
			"the leaf has lost groupReplace and is being token-UNIONed as a leaf-list. "+
			"Do NOT fix that by dropping multi: multi governs flat-set token "+
			"absorption, not validation, so dropping it leaves SchemaValidate green "+
			"while changing the compiled result and redding 14 NAT cells.", addr, port)
	}
}
