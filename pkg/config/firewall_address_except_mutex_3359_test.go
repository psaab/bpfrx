...
// AND an `except` prefix-list in the same direction. For a SPECIFIC positive
// match this shape has no faithful single-term representation in xpf (the
// direction would need both a positive set and a negated set), so it is rejected
// at commit. xpf's parser lands the literal addresses and the prefix-list
// references (positive AND except) on separate term fields, so both coexist; the
// userspace lowering then FOLDED the except prefixes into the positive set
// (dropping the except modifier) — a silent fail-OPEN for a discard/reject term
// (traffic the operator carved out via `except` was no longer dropped).
// validateFilterAddressExceptStrict makes the conflict an operator-visible
// commit error instead; the runtime fold is now positive-wins (fail-safe) for
// any leniently-loaded term.
//
// #4338: a MATCH-ANY positive (`source-address 0.0.0.0/0` / `::/0`) combined
// with an `except` prefix-list is the canonical Junos "match any EXCEPT the
// listed prefixes" lockdown idiom — Junos accepts it and so does xpf now (it
// composes to the sole-`except` representation). Only the SPECIFIC-positive +
// except case below is a genuine conflict. See
// firewall_address_except_matchany_4338_test.go for the accept-path proof.
//
...
func TestFilterSourceAddrPositiveAndExceptRejected_3359(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "source") ||
		!strings.Contains(err.Error(), "no faithful single-term representation") {
