...
func TestInsertBeforeAfter(t *testing.T) {
	tree := &ConfigTree{}
	setCommands := []string{"set security zones security-zone trust", "set security zones security-zone untrust", "set security policies from-zone trust to-zone untrust policy first match source-address any", "set security policies from-zone trust to-zone untrust policy first match destination-address any", "set security policies from-zone trust to-zone untrust policy first match application any", "set security policies from-zone trust to-zone untrust policy first then permit", "set security policies from-zone trust to-zone untrust policy second match source-address any", "set security policies from-zone trust to-zone untrust policy second match destination-address any", "set security policies from-zone trust to-zone untrust policy second match application any", "set security policies from-zone trust to-zone untrust policy second then permit", "set security policies from-zone trust to-zone untrust policy third match source-address any", "set security policies from-zone trust to-zone untrust policy third match destination-address any", "set security policies from-zone trust to-zone untrust policy third match application any", "set security policies from-zone trust to-zone untrust policy third then permit"}
	for _, cmd := range setCommands {
		path, err := ParseSetCommand(cmd)
		if err != nil {
			t.Fatalf("ParseSetCommand(%q): %v", cmd, err)
