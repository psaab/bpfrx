...
func runUniformGatesRoutingRibRPM(tree *ConfigTree, cfg *Config, opts compileOpts) error {
	...
	// port; the reth owns the units, addresses and security zone of the shared
	// device, and `ResolveReth` collapses the reth's rows onto the member's
	// netdev on exactly that basis. A member that names itself, names an
	// unconfigured parent, or carries its own logical units breaks the premise
	// the Rust egress-zone ledger defers on — the last of the three fails OPEN,
	// because an independently addressed member unit installs a connected route
	// on the shared ifindex while only the reth's row names a zone. Strict on
	// commit / commit-check (hard reject so the incoherence is operator-visible
	// before it mis-zones traffic); lenient on load / peer-sync (warn — #1960),
	// where the Go builder's base-row-only mark keeps a grandfathered member
	// unit voting and the ifindex fail-closed.
