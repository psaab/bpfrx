...
func TestBracedPackedUntouchedWithoutOptIn8886(t *testing.T) {
	aging := setSchema.children["security"].children["flow"].children["aging"]
	if aging == nil {
		t.Fatal("schema has no security/flow/aging — this cell would prove nothing")
	}
	if aging.packedStatements {
		t.Skip("aging has since opted in; pick another un-opted container for this control")
	}
	txt := `security { flow { aging { early-ageout 10 high-watermark 90; } } }`
	tree, perrs := NewParser(txt).Parse()
	if len(perrs) > 0 {
	...
	if cfg.Security.Flow.AgingHighWatermark != 0 {
		t.Errorf("an un-opted container split its packed run (high-watermark=%d). "+
			"The split must stay gated on packedStatements: making it global "+
