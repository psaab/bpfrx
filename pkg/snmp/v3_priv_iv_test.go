...
func buildV3AuthPrivAESRequest(t *testing.T, userName string, engineID, authKey, privKey []byte, msgBoots, msgTime, ivBoots, ivTime int) []byte {
	...
	// Encrypt as a manager would: IV from ivBoots/ivTime, unique salt supplied.
	privParams := testPrivSalt()
	enc, err := encryptAES128(privKey, scopedPDU, privParams, ivBoots, ivTime)
	if err != nil || enc == nil {
...
func TestAESDecryptIV_WrongIVFailsDecrypt(t *testing.T) {
	...
	pp := testPrivSalt()
	enc, err := encryptAES128(key, plaintext, pp, 5, 1000)
	if err != nil || enc == nil {
