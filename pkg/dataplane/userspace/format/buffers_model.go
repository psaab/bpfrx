...
// This file holds the buffer-status row/taxonomy MODEL for `show system
// buffers`. It is the single source that both renderers consume: the CLI/gRPC
// text formatter (FormatSystemBuffers in buffers.go) and the REST/structured
// exporter (StructuredSystemBufferRows in buffers.go) build from
// buildSystemBufferModel, so the sampling, aggregation, taxonomy labels, usage
// classification, and per-binding fallback live in exactly one place. The split
// mirrors the #4656 status_sections.go / #4657 cos_sections.go model-then-render
// pattern and closes the CLI/gRPC/REST buffer-status parity drift (#4661): a
// counter or capacity row added here surfaces in every renderer at once.

// Buffer-status taxonomy: the stable row-name labels shared by the text and
// structured renderers. Adding a row means adding a label here and wiring it
// into systemBufferRows / systemBufferCounterRows below.
const (
...
// systemBufferModel is the renderer-agnostic buffer-status snapshot: the
// bounded utilization rows, the unbounded status/pressure counter rows, and the
// AF_XDP known-binding denominators used by the "AF_XDP unavailable" hint. Both
// FormatSystemBuffers and StructuredSystemBufferRows build one via
// buildSystemBufferModel so they can never disagree about which rows exist.
type systemBufferModel struct {
	rows        []systemBufferRow
	counterRows []systemBufferCounterRow
	knownUMEM   int
	knownTX     int
}

// buildSystemBufferModel performs the single sampling + aggregation pass that
// backs every `show system buffers` renderer. It samples the helper status once
// and derives both the bounded utilization rows and the unbounded counter rows
// from that shared sample set.
func buildSystemBufferModel(status userspace.ProcessStatus, detail bool) systemBufferModel {
