...
ctrlReady:
	...
	for _, binding := range status.Bindings {
		...
		if binding.QueueID >= bindingQueuesPerIface {
			return m.failClosedUserspaceCtrlLocked(ctrlMap, ctrl, fmt.Errorf(
				"update userspace_bindings: queue-id=%d >= stride=%d would alias the adjacent ifindex queue-0 slot (ifindex=%d) — cap the helper queue count or raise BINDING_QUEUES_PER_IFACE in bpf/headers/xpf_common.h (#4894)",
	...
	for childIfindex, parentIfindex := range buildUserspaceIngressBindingAliases(m.lastSnapshot) {
		for _, binding := range status.Bindings {
			...
			if binding.QueueID >= bindingQueuesPerIface {
				return m.failClosedUserspaceCtrlLocked(ctrlMap, ctrl, fmt.Errorf(
					"update aliased userspace_bindings: queue-id=%d >= stride=%d would alias the adjacent ifindex queue-0 slot (child=%d parent=%d) — cap the helper queue count or raise BINDING_QUEUES_PER_IFACE in bpf/headers/xpf_common.h (#4894)",
