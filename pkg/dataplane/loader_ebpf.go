...
func (m *Manager) loadAllObjects() error {
	...
	m.maps["ipv4_flow_cache"] = zoneObjs.Ipv4FlowCache
	...
	replaceOpts.MapReplacements["rg_active"] = zoneObjs.RgActive
	replaceOpts.MapReplacements["ha_watchdog"] = zoneObjs.HaWatchdog
...
// loadUserspaceShimObjects loads the retained Rust XDP shim plus the shared
// maps that the AF_XDP userspace runtime still exchanges with Go. It must not
// load xdp_main, XDP tail-call programs, or TC programs.
