	...
	if len(leases4) > 0 {
		...
		if detail {
			...
			for _, l := range leases4 {
				...
				fmt.Printf("  %-18s %-20s %-15s %-10s %-12s %s\n",
					l.Address, sanitizeForDisplay(l.HWAddress), sanitizeForDisplay(l.Hostname), l.SubnetID, l.ValidLife, l.ExpireTime)
			}
		} else {
			fmt.Printf("  %-18s %-20s %-15s %-12s %s\n", "Address", "MAC", "Hostname", "Lifetime", "Expires")
			for _, l := range leases4 {
				fmt.Printf("  %-18s %-20s %-15s %-12s %s\n",
					l.Address, sanitizeForDisplay(l.HWAddress), sanitizeForDisplay(l.Hostname), l.ValidLife, l.ExpireTime)
			}
		}
	}
	if len(leases6) > 0 {
