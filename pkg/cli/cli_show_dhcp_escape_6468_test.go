...
// seam pointing at the malicious lease set.
func newEscapeLeaseCLI(t *testing.T) *CLI {
	t.Helper()
	dir := t.TempDir()
...
`); err != nil {
	...
	return &CLI{
		store:        store,
		dhcpServerFn: func() *dhcpserver.Manager { return escapeLeaseManager(t) },
	}
}

func TestShowDHCPServer_LocalCLIEscapesHostname_6468(t *testing.T) {
	for _, detail := range []bool{false, true} {
		c := newEscapeLeaseCLI(t)
