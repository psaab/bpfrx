...
| `status_parse.go` | Parsed Get* methods + their public types (`RIPRouteEntry`, `ISISAdjacency`, `OSPFNeighbor`, `BGPPeerSummary`, `BGPRoute`, `FRRRouteDetail`, `FRRNextHop`) + `parseRouteJSON`, `parseBGPSummaryJSON`, `FormatRouteDetail`. **`GetBGPSummary` parses `show bgp summary json`** (structured JSON, `parseBGPSummaryJSON`), NOT the text table — see the #3942 note below. |
