...
// TestWrapSyncConnInstallsKeysDirectionally7163 binds the site where the
// directional property is REALIZED, as opposed to where it is derived.
//
// #7163's tests bind the derivation: performSyncHandshake returns two
// independent keys, and the in-place upgrade installs each at its own frame
// boundary. Nothing bound the CONNECT path's hand-off — the two assignments in
// wrapSyncConn that copy those derived keys onto the authConn the frame layer
// actually uses. Verified by mutation on the merged tree: changing
// `ac.writeKey = keys.writeKey` to `ac.writeKey = keys.readKey` — the
// pre-#7163 undirected shape, at the assignment site — left the whole
// pkg/cluster and pkg/daemon suites GREEN.
//
// The reason it escaped is worth naming, because it is a recurring shape: the
// only test that called wrapSyncConn asserted `wrapped.authed()`, and authed()
// is `readAuthed() && writeAuthed()`, i.e. both keys NON-EMPTY. An existence
// check standing in for a value check passes for every wrong value that is
// still a value.
//
// What that mutation costs is not hypothetical either. It makes readKey ==
// writeKey on a connect-authenticated connection, which is exactly the
// self-fencing primitive #7163 exists to remove: a frame the node SENT
// verifies when echoed straight back at it on the same connection, and the
// anti-replay watermark cannot see it because it tracks the RECEIVE counter.
// The worst reflectable frame is syncMsgFence — empty payload, and on receipt
// the victim disables every redundancy group it owns.
//
// So this asserts the CONSEQUENCE, not the assignment: the echo is refused,
// with a positive control that the same frame verifies at the peer. A test
// that only compared byte slices would be satisfied by any two distinct
// values, including two that are wrong in the same direction.
func TestWrapSyncConnInstallsKeysDirectionally7163(t *testing.T) {
	key := []byte("wrap-directional-psk-7163")
	...
	// Distinguishable, and NOT symmetric: swapping them must be detectable.
	keys := syncNoiseKeys{
		readKey:  bytes.Repeat([]byte{0xA1}, syncNoiseKeyLen),
		writeKey: bytes.Repeat([]byte{0xB2}, syncNoiseKeyLen),
	}
	ac := s.wrapSyncConn(0, ca, syncAuthAuthenticated, keys)

	if !bytes.Equal(ac.writeKey, keys.writeKey) {
		t.Fatalf("wrapSyncConn installed the wrong WRITE key: got %x, want %x. The "+
			"handshake derived a directional pair and this is where it reaches the "+
			"frame layer; getting it wrong here undoes the derivation.",
			ac.writeKey, keys.writeKey)
	}
	if !bytes.Equal(ac.readKey, keys.readKey) {
		t.Fatalf("wrapSyncConn installed the wrong READ key: got %x, want %x",
			ac.readKey, keys.readKey)
	}

	// THE CONSEQUENCE. A frame this connection sealed must not verify when
	// echoed back to it — that echo is the vector-B self-fencing primitive.
	peer := &authConn{Conn: cb, readKey: keys.writeKey, writeKey: keys.readKey}
	fence := encodeRawMessage(syncMsgFence, nil)
	sealed := ac.sealFrame(fence)
	header := sealed[:syncHeaderSize]
	n := binary.LittleEndian.Uint32(header[8:12])
	payload := sealed[syncHeaderSize : syncHeaderSize+int(n)]
	trailer := sealed[syncHeaderSize+int(n):]

	if err := ac.verifyFrame(header, payload, trailer); err == nil {
		t.Fatal("a connect-authenticated connection VERIFIED its own sealed frame when " +
			"echoed back at it. That is the vector-B self-fencing primitive: the frame " +
			"is syncMsgFence, and on receipt this node disables every redundancy group " +
			"it owns. The anti-replay watermark cannot see it — it tracks the RECEIVE " +
			"counter, which is independent of the send counter.")
	}
	// Positive control: the refusal above must mean "wrong direction", not
	// "sealing is broken".
	if err := peer.verifyFrame(header, payload, trailer); err != nil {
		t.Fatalf("the same frame must verify at the PEER, or the refusal above proves "+
			"nothing about directionality: %v", err)
	}
}

// TestWrapSyncConnLeavesAnUnauthenticatedConnPassThrough7163 binds the other
// arm of the same function. An unkeyed node never handshakes and must be
// byte-for-byte a legacy peer; installing a key here would make it start
// sealing frames its peer is not verifying, which drops the connection.
func TestWrapSyncConnLeavesAnUnauthenticatedConnPassThrough7163(t *testing.T) {
	key := []byte("wrap-passthrough-psk-7163")
	s := newAuthSyncNode(t, key, 0)
	...
	// Keys are supplied but the mode says unauthenticated: the mode decides,
	// not the presence of key material.
	keys := syncNoiseKeys{
		readKey:  bytes.Repeat([]byte{0xC3}, syncNoiseKeyLen),
		writeKey: bytes.Repeat([]byte{0xD4}, syncNoiseKeyLen),
	}
	ac := s.wrapSyncConn(0, ca, syncAuthUnauthenticated, keys)

	if ac.readAuthed() || ac.writeAuthed() {
		t.Fatalf("an UNAUTHENTICATED wrap installed key material (read=%v write=%v). "+
			"The connection would start sealing frames the peer is not verifying, and "+
			"the peer would read the trailer as stream bytes and drop the connection.",
			ac.readAuthed(), ac.writeAuthed())
	}
	if ac.authPSK != nil {
		t.Fatal("an unauthenticated connection must not record an auth PSK, or the " +
			"#6628 reconciler will treat it as already upgraded and never upgrade it")
	}
}
